...
<Tabs>
  ...
  <Tab title="Synthetic benchmarks">
    ...
    |:-------------|-------:|--------:|--------:|------:|------:|
    | 1,000 files | **167ms** | 714ms | 347ms | **4.3x** | **2.1x** |
    | 5,000 files | **313ms** | 932ms | 412ms | **3.0x** | **1.3x** |
  </Tab>
</Tabs>

<Info>
  Benchmarked on real-world open-source projects from 174 to 38,146 files against fallow 2.91.0, knip 5.87.0, and knip 6.6.1 on Apple M5, 32 GB RAM, Node 22. Median of 5 cold runs (no cache) with 2 warmups. fallow is fastest on small-to-medium projects (roughly 5-18x vs knip v5, 2.7-9x vs knip v6). On large projects, knip v6's Oxc parser is competitive or faster (astro, TypeScript); fallow's advantage there is doing more in one tool. fallow's persistent cache makes repeat (warm) runs faster again, the table uses the conservative cold numbers.
...
<CardGroup cols={2}>
  <Card title="More plugins" icon="puzzle-piece">
    155 plugins vs 121. Knip covers more niche, legacy, and ecosystem-specific tooling.
  </Card>
...
| Speed vs knip v6 | **2.7-9x faster** on small-to-medium projects, competitive on large | Baseline |
