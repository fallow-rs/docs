...
<Tabs>
  <Tab title="Real-world projects">
    ...
    | [vite](https://github.com/vitejs/vite) | 1,420 | **322ms** | 302ms | 155ms | 0.9x | 0.5x |
...
<Info>
  Benchmarked on 7 real-world open-source projects from 174 to 20,416 files against fallow 2.46.0, knip 5.87.0, and knip 6.6.1 on Apple M5, 32 GB RAM, Node 22. Median of 5 runs with 2 warmups. On small-to-medium projects, fallow is 5-34x faster than knip v5 and 2-13x faster than knip v6. On the largest monorepos (next.js, vue/core), knip errors out without producing results; fallow is the only tool that completes. On vite specifically, knip v6 is faster than fallow (155ms vs 322ms) because knip's dedicated vite plugin narrows entry points more aggressively than fallow's general resolver. TanStack/query is omitted pending investigation of a regression in fallow 2.46.0. * knip errors on next.js and vue/core (exits without valid results).
</Info>
