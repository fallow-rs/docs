...
<Tabs>
  ...
  <Tab title="Synthetic benchmarks">
    ...
    | 5,000 files | **201ms** | 646ms | 340ms | **3x** | **2x** |
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Details">
    ...
    **Knip extras:**
    - **Circular dependencies** -- Detected via the knip Language Server.
...
| Plugins with config parsing | 30 | -- |
...
<Tabs>
  ...
  <Tab title="Fallow-only plugins">
    Rspack, Tsdown, Drizzle, Knex, Wrangler (Cloudflare), Expo, React Native, lefthook, Docusaurus, and others.
  </Tab>
  <Tab title="Knip-only plugins">
    Knip covers additional tools like Capacitor, Wireit, typedoc, c8/nyc, markdownlint, Remark, GitHub Actions, Netlify, and several other niche integrations. Check the [knip plugin list](https://knip.dev/reference/plugins) for the full inventory.
  </Tab>
</Tabs>
...
- **MCP server** -- AI agent integration via the Model Context Protocol for tools like Claude Code.
...
<CardGroup cols={2}>
  ...
  <Card title="Circular dependencies" icon="arrows-spin">
    Circular dependency detection is available through the knip Language Server.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Circular dependencies" icon="arrows-spin">
    You specifically need circular dependency detection as part of your dead code workflow.
  </Card>
