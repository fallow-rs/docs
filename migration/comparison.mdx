...
Cold runs (no cache) so both tools work from scratch. Fastest tool per row in bold.
...
- **Cross-reference analysis**: `fallow dead-code --include-dupes` finds dead code that is also duplicated, showing high-priority cleanup targets.

## Features knip has that fallow doesn't

<CardGroup cols={2}>
  <Card title="More plugins" icon="puzzle-piece">
    Knip covers more niche, legacy, and ecosystem-specific tooling.
  </Card>
