...
Fallow reads `exposes` and `remotes` from `module-federation.config.{ts,js,mjs,cjs,mts,cts}`, and from a Federation plugin call in a webpack, rspack, rsbuild, vite or `next.config.*` file. The recognised plugin calls are `ModuleFederationPlugin`, `NextFederationPlugin`, `moduleFederationPlugin`, `pluginModuleFederation`, and `federation`. A call is read wherever it sits in the config program. A call counts only when its first argument is an object that declares `exposes` or `remotes`.
...
- The array form of `exposes`. A string element is both the public name and the module request. An object element goes through the object reader.
...
- A spread, a value that is not a static object literal, and an array element that holds glob syntax, a nested array or a value that is not a string.

Reading is syntactic. No remote container is fetched and nothing about the remote is validated: its availability, its exports, versions, and shared singletons are all out of scope.

A declaration fallow could not read in full reaches the report as a `plugin-config-unreadable` entry in `workspace_diagnostics[]`, and prints on stderr. The entry names the plugin, the key and the reason, with the config file in `path`. Its `reason` is `not-object-literal`, `array-form`, `spread` or `unreadable-entries`; treat that set as open. The plugin is `module-federation` for a standalone config file, and `webpack`, `rspack`, `rsbuild`, `vite` or `nextjs` for options read from that plugin's own config. Two unreadable keys in one file give two entries. The entry carries `degrades_analysis`, so both shipped CI integrations report it under degraded inputs.
