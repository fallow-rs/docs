...
<Accordion title="Combined detection (boolean logic)">
  Combine conditions with `all` (AND), `any` (OR), or `not`:
...
| `when` | object | Gate for the manifest: a map from dotted field path to an expected value. Fallow processes the manifest only when ALL values match by strict equality. An empty map matches every manifest |
...
`when` matching uses **strict equality**. `{ "plugin.browser": true }` matches a manifest whose `plugin.browser` is literally `true`. Fallow skips that entry for a manifest with `plugin.browser: false`.
