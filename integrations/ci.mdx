  ...
  <Tab title="GitHub Action">
    <Steps>
      ...
      <Step title="Configure inputs">
        ...
        | `fail-on-stale-baseline` | `false` | Fail the job when the loaded `baseline` has entries that match nothing this run, so a baseline cannot rot unnoticed. Independent of `fail-on-issues`: a baseline whose entries all went stale on a project that is now clean reports zero issues. On a pull request the action re-runs the baseline comparison once over the whole project so the gate can still apply; when the run cannot judge the baseline at all (`production`, `workspace`, `changed-workspaces`, `issue-types`, or scoping passed through `args`) it warns and stands down instead of passing silently. |
  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_FAIL_ON_STALE_BASELINE` | `"false"` | Fail the pipeline when the loaded `FALLOW_BASELINE` has entries that match nothing this run. Independent of `FALLOW_FAIL_ON_ISSUES`: a fully stale baseline on a project that is now clean reports zero issues. In merge-request pipelines the template re-runs the baseline comparison once over the whole project so the gate can still apply; when the run cannot judge the baseline at all (`FALLOW_PRODUCTION`, `FALLOW_WORKSPACE`, `FALLOW_CHANGED_WORKSPACES`, `FALLOW_ISSUE_TYPES`, or scoping passed through `FALLOW_ARGS`) it warns and stands down. |
...
A run that loads a baseline reports how much of that baseline still matches, and warns in the log and the job summary when entries have gone stale. Set `fail-on-stale-baseline` (GitHub) or `FALLOW_FAIL_ON_STALE_BASELINE` (GitLab) to turn that into a failing build. The PR comment and the MR note do not carry the advisory yet.
