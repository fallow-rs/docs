  ...
  <Tab title="GitHub Action">
    <Steps>
      ...
      <Step title="Configure inputs">
        ...
        | `fail-on-stale-baseline` | `false` | Fail the job when the loaded `baseline` has entries that match nothing this run, or when it is a baseline of another `kind`. The staleness warning itself needs no input. Any run that loads a baseline reports it. A narrowed run re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This input only decides whether that verdict fails the job. It is independent of `fail-on-issues`: a baseline whose entries all went stale on a project that is now clean reports zero issues. When the run still cannot cover the whole project (`production`, `workspace`, `changed-workspaces`, or a positional path passed through `args`) the gate warns and stands down. |
        ...
        | `args` | -- | Additional arguments to pass to fallow. The action rejects `--fail-on-stale-baseline` with exit 2 when `command: audit`. An audit analyzes only the files that changed against its base, so it cannot judge a whole-project baseline. |
      </Step>
  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_FAIL_ON_STALE_BASELINE` | `"false"` | Fail the pipeline when the loaded `FALLOW_BASELINE` has entries that match nothing this run, or when it is a baseline of another `kind`. The staleness warning itself needs no variable. Any run that loads a baseline reports it. A narrowed pipeline re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This variable only decides whether that verdict fails the pipeline. It is independent of `FALLOW_FAIL_ON_ISSUES`: a fully stale baseline on a project that is now clean reports zero issues. When the run still cannot cover the whole project (`FALLOW_PRODUCTION`, `FALLOW_WORKSPACE`, `FALLOW_CHANGED_WORKSPACES`, or a positional path passed through `FALLOW_ARGS`) the gate warns and stands down. |
        ...
        | `FALLOW_ARGS` | -- | Additional arguments (space-separated). The template rejects `--fail-on-stale-baseline` with exit 2 when `FALLOW_COMMAND=audit`. An audit analyzes only the files that changed against its base, so it cannot judge a whole-project baseline. |
...
The step outputs `gates-failed`, `gates-warned`, `gates-skipped` and `gates-passed` carry the comma-separated names. `analysis-degraded` reports whether the findings cover less than the whole project. `requests-unapplied` names the requests the run could not apply, each with its reason in parentheses, for example `changed-since (git-failed)`. `baseline-path` carries the baseline the run compared against, as the `baseline` input spelled it. It stays empty for a baseline passed through `args`. GitLab writes the same names, and `FALLOW_REQUESTS_UNAPPLIED`, to `fallow-gates.env`, and publishes that file through `artifacts:reports:dotenv`.

The sticky PR comment, the MR note, the inline review bodies and the Check Run carry the same verdict: the baseline advisory as a blockquote note, plus one row per armed gate. Each row gives what the gate observed and the threshold it compared against. A gate the run did not enforce renders neutral, so the Check Run conclusion is unchanged. To make each gate its own required check, post the decision sidecar yourself with `fallow ci post-check-run --split-gates`. That flag creates one `Fallow / <gate>` status context per gate instead of a single `Fallow` check. Neither integration passes it.
...
On a fallow older than 3.28.0 the envelope carries no `request_outcomes`, and `baseline_staleness` carries no `scope_reasons` or `unrecognised_format`. `requests-unapplied`, `baseline-scope-reasons` and `baseline-unrecognised` stay empty there, and the warnings that read them stay silent. The whole-project baseline re-read then reads the `production`, `workspace` and `changed-workspaces` inputs instead.

### Degraded and empty analysis

A run whose findings cover less than the whole project warns once, as `Fallow ran with degraded inputs:` and then the diagnostic kinds with their counts. Both integrations select on `degrades_analysis` in `workspace_diagnostics`, so they read a kind added in a later release as it is. A run that analyzed no source file gets its own sentence. Its clean result means the run measured nothing, not that it found nothing. Such a run warns and passes by default. Set `fail-on-empty-analysis` or `FALLOW_FAIL_ON_EMPTY_ANALYSIS` to fail instead.

When the run cannot apply a request that narrows the report, it warns as `Fallow could not apply:` and then names each request with its reason. The report then covers more of the project than you asked for, so do not read such a run as scoped to the change. The selector is `affects: "scope"`, so a request that writes a file beside the report never appears in this warning. See [`request_outcomes`](/cli/global-flags) for the request names, statuses and reasons.

The action warns separately when `format: sarif` or `sarif: true` produced no SARIF document. That warning says that the action uploads nothing, and that code scanning keeps the alerts of the previous upload. It gives the reason from `request_outcomes["sarif-file"]` when the binary published one.

The opposite case warns as `Fallow applied <request> over an empty scope`. The run applied the request, and it measured the remaining scope as empty. No finding can survive a scope that measured nothing, so a clean report there covered nothing. Check the diff or the ref you gave the run before you read the result as clean. Both integrations key on `scope_size: 0` beside `status: "applied"`, and on `affects: "scope"`, so a file written beside the report never triggers this warning. The rendered comment, review, job summary and annotation bodies carry the same fact, as a clause on their request-outcome line.

A run that loads a baseline reports how much of that baseline still matches. It warns in the log and the job summary when entries have gone stale. A narrowed run gets the baseline re-read over the whole project first, because a scoped run cannot judge a whole-project baseline. `baseline_staleness.scope_reasons` decides that. The integrations re-read when they added every channel that narrowed the run themselves: `diff`, `changed-since`, `changed-files`, `scope`, `file` and `issue-type-filter`. For `production`, `workspace` and `changed-workspaces` they stand down and name the channel, because those state what you think the project is. The same rule holds for a scope that reaches the run through `args` or `FALLOW_ARGS`. The action publishes the list as `baseline-scope-reasons`. Set `fail-on-stale-baseline` (GitHub) or `FALLOW_FAIL_ON_STALE_BASELINE` (GitLab) to turn the staleness verdict into a failed build. The baseline input and the save-baseline input must not name the same file, because the run saves before it compares. The integrations warn when they see that. The PR comment and the MR note carry the gate line `fallow report` renders, which states what each gate concluded.

A baseline that the command cannot read as its own (`baseline_staleness.unrecognised_format`) suppresses nothing. It is a baseline another command saved, or a file with nothing of this command in it. Fallow does not report a baseline you correctly saved on a project with nothing to record. `dead-code`, `dupes` and `health` all report it, and all keep their exit code. The step log, the job summary, the PR comment, the MR note and the Check Run carry the sentence. The action publishes `baseline-unrecognised`, and the job summary names the file through `baseline-path`. The baseline gate trips on such a file, which `baseline-gate-trips` also reports, so `fail-on-stale-baseline` fails the job there. Each command has its own baseline format: see [dead-code](/cli/dead-code), [dupes](/cli/dupes) and [health](/cli/health).

`command: audit` judges none of the baselines it loads, because it analyzes only the files that changed against its base. It prints one notice per loaded baseline, with the entry count and the unscoped command that can judge it. It rejects `--fail-on-stale-baseline` in `args` or `FALLOW_ARGS` with exit 2. See [audit](/cli/audit) for the per-analysis baseline inputs.

#### Where each advisory lands

A change-scoped run says nothing about staleness, because it cannot judge a whole-project baseline. The PR comment, the MR note and the Check Run render from that run's own envelope. They carry no baseline advisory on a pull request. The step log and the job summary carry the verdict of the whole-project re-read. The baseline rows below describe a run that could judge what it loaded. The job summary and the Check Run are GitHub only. On GitLab, read the job log, the MR note and the Code Quality report.
...
The `stale-baseline` gate that stands down reaches every surface through the gate inventory. Only the sentence that says why, and which channel narrowed the run, stays in the log.
