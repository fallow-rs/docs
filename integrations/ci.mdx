  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_PRODUCTION` | `false` | Enable production mode for every analysis |
        | `FALLOW_PRODUCTION_DEAD_CODE` | `false` | Combined mode only: per-analysis production mode for dead-code |
        | `FALLOW_PRODUCTION_HEALTH` | `false` | Combined mode only: per-analysis production mode for health |
        | `FALLOW_PRODUCTION_DUPES` | `false` | Combined mode only: per-analysis production mode for duplication |
