  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_MIN_SCORE` | `""` | `FALLOW_COMMAND: health` only: fail when the health score drops below this threshold (0-100). Implies `--score`, so the template adds `--complexity` unless a health section variable is set, keeping the Code Quality report and the MR note populated. `FALLOW_FAIL_ON_ISSUES` still counts every finding. |
