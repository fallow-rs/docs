...
The step outputs `gates-failed`, `gates-warned`, `gates-skipped` and `gates-passed` carry the comma-separated names, and `analysis-degraded` reports whether the findings were computed over less than the whole project. `requests-unapplied` names the narrowing requests the run could not apply, each with its reason in parentheses, for example `changed-since (git-failed)`. GitLab writes the same names to `fallow-gates.env`, including `FALLOW_REQUESTS_UNAPPLIED`, and publishes that file through `artifacts:reports:dotenv`.
...
A baseline that carries no key the running command writes (`baseline_staleness.unrecognised_format`) suppresses nothing, so the advisory and the gate both read green on a file that is doing no work. It is a baseline another command saved, or a file with nothing of this command in it; a baseline correctly saved on a project with nothing to record is not reported. The step log and the job summary say so, and the action publishes `baseline-unrecognised`. The PR comment and the MR note do not carry this line, so read the log when a baseline looks inert. `dupes` and `health` report it; `dead-code` rejects a foreign baseline at load with exit 2 instead. Baseline formats are per command; see [dead-code](/cli/dead-code), [dupes](/cli/dupes) and [health](/cli/health).
...
| Baseline recognises nothing | Yes | Yes | No | No |
