  ...
  <Tab title="GitHub Action">
    <Steps>
      ...
      <Step title="Configure inputs">
        ...
        | `fail-on-stale-baseline` | `false` | Fail the job when the loaded `baseline` has entries that match nothing this run. The staleness warning itself needs no input: any run that loads a baseline reports it, and a run scoped to changed files re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This input only decides whether that verdict fails the job. Independent of `fail-on-issues`: a baseline whose entries all went stale on a project that is now clean reports zero issues. When the run still cannot be widened (`production`, `workspace`, `changed-workspaces`, or a positional path passed through `args`) it warns and stands down instead of passing silently. |
  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_FAIL_ON_STALE_BASELINE` | `"false"` | Fail the pipeline when the loaded `FALLOW_BASELINE` has entries that match nothing this run. The staleness warning itself needs no variable: any run that loads a baseline reports it, and a merge-request pipeline re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This variable only decides whether that verdict fails the pipeline. Independent of `FALLOW_FAIL_ON_ISSUES`: a fully stale baseline on a project that is now clean reports zero issues. When the run still cannot be widened (`FALLOW_PRODUCTION`, `FALLOW_WORKSPACE`, `FALLOW_CHANGED_WORKSPACES`, or a positional path passed through `FALLOW_ARGS`) it warns and stands down. |
...
The step outputs `gates-failed`, `gates-warned`, `gates-skipped` and `gates-passed` carry the comma-separated names, and `analysis-degraded` reports whether the findings were computed over less than the whole project. GitLab publishes the same names through `artifacts:reports:dotenv`.
