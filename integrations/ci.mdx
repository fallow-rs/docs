...
<Tabs>
  ...
  <Tab title="Manual Setup">
    ...
    ```json
    {
      ...
      "comments": [
        ...
        },
        {
          ...
          "line": 5,
          "side": "RIGHT",
          "body": "**error** `fallow/unused-dependency`: ...\n\n**warn** `fallow/unused-dev-dependency`: ...\n\n<!-- fallow-fingerprint:v2: linecomp:9a18387f430bc48d -->",
          "fingerprint": "linecomp:9a18387f430bc48d",
          "constituent_fingerprints": ["1d04d08d81daf650", "2a4393b133ea1523"]
        }
      ],
      "marker_regex": "(?m)^<!-- fallow-fingerprint:v2: ((?:[a-z]+:)?[0-9a-f]{16}) -->\\s*$",
      "meta": {
    ...
    Every entry under `comments[]` is fully rendered Markdown, ready to POST as-is. The `<!-- fallow-fingerprint:v2: <fingerprint> -->` marker embedded in each body is the stable identifier used for reconciliation across runs. The top-level `marker_regex` is the canonical extraction pattern; running it (with one capture group) against any existing PR/MR comment yields the fingerprint string. The `(?m)` inline-multiline flag makes the regex work in both Rust's `regex` crate and JavaScript ES2018 RegExp without flag-awareness on the consumer side.
    ...
    When multiple findings collide on the same `path:line` (e.g., three `unused-dependency` variants on `package.json:5`), they collapse into one comment with stacked body paragraphs and a stable `fingerprint = "linecomp:<16-hex of path:line>"` keyed on `(path, line)`. The primary fingerprint stays stable across runs even when constituent findings change membership, enabling update-in-place reconciliation that preserves reviewer reply threads. The `constituent_fingerprints` array carries the per-finding v1-style fingerprints in stable sort order so consumers detect content change between runs. Single-finding comments keep the bare 16-hex `fingerprint` shape and omit `constituent_fingerprints` entirely.
    ...
    To avoid duplicate threads on later pushes, fetch the existing MR discussions before POSTing and extract their fingerprints with the regex fallow emits in the envelope's `marker_regex` field. The capture group yields the fingerprint string; consumers compare against `comments[].fingerprint` to filter to new entries:
    ...
    # Extract fingerprints from existing discussion bodies via the
    # marker_regex fallow emits in plan.marker_regex (one capture group).
    # The regex uses (?m) inline-multiline, so it works in both Rust and JS.
    marker_regex=$(jq -r '.marker_regex' plan.json)
    existing_fps=$(echo "$existing" | jq -r '.[].notes[].body? // empty' \
      | sed -nE "s|.*${marker_regex}.*|\\1|p" \
      | sort -u)
    ...
    **Stable-fingerprint reconciliation for merged comments**: when same-(`path`, `line`) findings collapse to one comment with a `linecomp:<...>` primary fingerprint, the comment's fingerprint stays stable across runs even when constituent findings change membership. Consumers detect content change by diffing the `constituent_fingerprints` array between runs and PATCH the comment body in place via the GitLab discussion-notes API (preserving reviewer reply threads on the thread) instead of delete-and-recreate.
