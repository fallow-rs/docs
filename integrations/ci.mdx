...
<Tabs>
  <Tab title="GitHub Action">
    <Steps>
      ...
      <Step title="Configure inputs">
        ...
        | `fail-on-stale-baseline` | `false` | Fail the job when the loaded `baseline` has entries that match nothing this run, or when it is a baseline of another `kind`. The staleness warning itself needs no input: any run that loads a baseline reports it, and a narrowed run re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This input only decides whether that verdict fails the job. Independent of `fail-on-issues`: a baseline whose entries all went stale on a project that is now clean reports zero issues. When the run still cannot be widened (`production`, `workspace`, `changed-workspaces`, or a positional path passed through `args`) it warns and stands down instead of passing silently. |
        | `min-score` | -- | `command: health` only: fail when the health score drops below this threshold (0-100). Implies `--score`, so the action adds `--complexity` unless you set a health section input, keeping annotations, SARIF and the PR comment populated; `target_thresholds` and `hotspot_summary` are not restored. When this is set the CLI turns its own findings rule off and the count gate stands down with it, so the score alone decides the run. Rejected with exit 2 on any other command. |
        | `fail-on-empty-analysis` | `false` | Fail the job when the run analyzed no source file at all, so every count it reports is zero because nothing was measured rather than because the project is clean. Warns and passes by default, so a repository whose scope legitimately holds no source keeps passing. |
        ...
        | `review-id` | -- | Stable identifier (1-64 characters, `A-Za-z0-9._-`) that isolates inline review comments when multiple fallow review jobs target the same pull request. See [Isolating parallel review jobs](#isolating-parallel-review-jobs). |
        ...
        | `args` | -- | Additional arguments to pass to fallow. `--fail-on-stale-baseline` is rejected with exit 2 when `command: audit`, which analyzes only the files that changed against its base and cannot judge a whole-project baseline. |
      </Step>
  ...
  <Tab title="GitLab CI">
    <Steps>
      ...
      <Step title="Configure variables">
        ...
        | `FALLOW_TYPE_AWARE` | `""` | Set to `"true"` to enable semantic evidence, `"false"` to disable it, or leave empty to defer to repository config. |
        | `FALLOW_TYPE_AWARE_PROJECTS` | `""` | Comma-separated tsconfig paths. Empty uses automatic project discovery. Each path is forwarded as a separate CLI flag. |
        | `FALLOW_TYPE_AWARE_REQUIRE` | `""` | `best-effort` or `complete`. Empty defers to repository config; `complete` fails on partial or unavailable semantic evidence. |
        ...
        | `FALLOW_FAIL_ON_STALE_BASELINE` | `"false"` | Fail the pipeline when the loaded `FALLOW_BASELINE` has entries that match nothing this run, or when it is a baseline of another `kind`. The staleness warning itself needs no variable: any run that loads a baseline reports it, and a narrowed pipeline re-reads the baseline over the whole project first, because a scoped run cannot judge a whole-project baseline. This variable only decides whether that verdict fails the pipeline. Independent of `FALLOW_FAIL_ON_ISSUES`: a fully stale baseline on a project that is now clean reports zero issues. When the run still cannot be widened (`FALLOW_PRODUCTION`, `FALLOW_WORKSPACE`, `FALLOW_CHANGED_WORKSPACES`, or a positional path passed through `FALLOW_ARGS`) it warns and stands down. |
        | `FALLOW_MIN_SCORE` | `""` | `FALLOW_COMMAND: health` only: fail when the health score drops below this threshold (0-100). Implies `--score`, so the template adds `--complexity` unless a health section variable is set, keeping the Code Quality report and the MR note populated. When this is set the CLI turns its own findings rule off and the count gate stands down with it. |
        | `FALLOW_FAIL_ON_EMPTY_ANALYSIS` | `"false"` | Fail the pipeline when the run analyzed no source file at all. Warns and passes by default. |
        | `FALLOW_SECURITY_GATE` | `""` | Security delta gate for `FALLOW_COMMAND: "security"`. `new` fails only on changed-line candidates; `newly-reachable` fails only on candidates that became reachable from entry points (needs a base ref via `FALLOW_CHANGED_SINCE` or MR auto-detection). Gated security failures exit with code 8. MR comment and review renderers do not support security envelopes yet and are skipped for this command. |
        ...
        | `FALLOW_REVIEW_ID` | -- | Stable identifier (1-64 characters, `A-Za-z0-9._-`) that isolates inline review discussions when multiple fallow review jobs target the same MR. See [Isolating parallel review jobs](#isolating-parallel-review-jobs). |
        | `FALLOW_SUMMARY_SCOPE` | `all` | Sticky MR summary scope. Use `diff` to apply the diff filter to project-level dependency/catalog/override findings too. Inline review discussions are unaffected |
        | `FALLOW_PR_COMMENT_LAYOUT` | `default` | Sticky MR comment layout: `default`, `compact`, `gate-only`, or `details`. Use `gate-only` when Code Quality is the primary MR surface |
        ...
        | `FALLOW_SKIP_INSTALL` | `""` | Set to `"true"` to skip `npm install -g fallow` and use the `fallow` already resolvable on the job shell's `PATH` (fails fast if none is found). See the setup gotchas step below. |
        | `FALLOW_DIFF_FILTER` | `added` | Filter line-level findings to added diff hunks; use `diff_context`, `file`, or `nofilter` to widen review scope. |
        ...
        | `FALLOW_COVERAGE_ROOT` | -- | Rebase Istanbul file paths before matching (health, audit, and default combined commands). Use when coverage was generated under a different checkout root in CI / Docker. |
        ...
        | `FALLOW_ARGS` | -- | Additional arguments (space-separated). `--fail-on-stale-baseline` is rejected with exit 2 when `FALLOW_COMMAND=audit`, which analyzes only the files that changed against its base and cannot judge a whole-project baseline. |
      ...
      <Step title="Rich MR comments">
        **Summary comment**: Set `FALLOW_COMMENT: "true"` to post a rich MR comment with collapsible sections for dead code, duplication, and complexity findings. The comment is updated on each push (no spam). Set `FALLOW_SUMMARY_SCOPE: "diff"` when the sticky summary should hide pre-existing project-level dependency/catalog/override findings outside the diff. Set `FALLOW_PR_COMMENT_LAYOUT: "gate-only"` when GitLab Code Quality is the main MR surface and the sticky comment should stay compact.
      ...
      <Step title="Setup gotchas">
        ...
        The job then skips `npm install -g fallow` and runs your exact binary, so CI matches your local lint gate. If you also enable `FALLOW_COMMENT` / `FALLOW_REVIEW` and your binary is a dev build whose `--version` is not an exact release (for example `2.3.0-dev`), pin `FALLOW_SCRIPTS_REF` to a real tag or vendor `ci/scripts/` so the MR-integration scripts can still be fetched.
      </Step>
    </Steps>
  ...
  <Tab title="Manual Setup">
    ...
    | `FALLOW_REVIEW_GUIDANCE` | Both | Adds collapsed "What to do" guidance blocks to each inline finding when set to a truthy value. |
    | `FALLOW_SUMMARY_SCOPE` | Both | Controls sticky PR/MR summary scope only: `all` keeps project-level findings outside the diff, `diff` applies the diff filter to them too. |
    | `FALLOW_PR_COMMENT_LAYOUT` | Both | Controls sticky PR/MR summary layout: `default`, `compact`, `gate-only`, or `details`. |
  ...
  <Tab title="Other CI">
    Fallow works in any CI environment that can run Node.js or [install fallow](/installation). The pattern is the same everywhere:
...
Do not set this in normal CI configurations. The public key fingerprint and the manual out-of-band verification recipe live in [`SECURITY.md`](https://github.com/fallow-rs/fallow/blob/main/SECURITY.md) on the main repo. The VS Code extension performs its own Ed25519 verification on its auto-downloaded binary; see [the extension docs](/integrations/vscode) for details.
...
Adopting fallow on a codebase with existing findings? Baselines report only new issues; see [PR enforcement in the adoption guide](/adoption#7-turn-on-pr-enforcement).

### Gates

Every gate the run armed publishes a verdict in the analysis envelope, and both integrations read that rather than the CLI exit code, which they discard whenever stdout parses as JSON. A gate fails the build when three things hold: the input or variable that owns it asked for it, the CLI concluded `fail`, and the CLI marked the verdict enforced.

`fail-on-regression` owns the regression gate, `threshold` the duplication gate, `min-score` and `min-severity` the two health gates, `security-gate` the security gate, `fail-on-stale-baseline` the baseline gate, and `type-aware-require` the semantic completeness gate. Each is independent of `fail-on-issues` / `FALLOW_FAIL_ON_ISSUES`, which keeps its own job of gating on the issue count. `command: audit` is the exception and still gates on its verdict through `fail-on-issues`, so an audit job with `fail-on-issues: false` remains a reporting configuration.

A gate that concluded `fail` without its input being set warns and never fails, so a flag passed through `args` cannot override `fail-on-issues: false`. A gate that stood down without judging the run warns when its input asked for it. Every failing gate prints its own error line and the step exits once at the end, after the outputs and artifacts are written, so the comment, annotation and summary steps still run. The security gate keeps its documented exit code 8.

The step outputs `gates-failed`, `gates-warned`, `gates-skipped` and `gates-passed` carry the comma-separated names, and `analysis-degraded` reports whether the findings were computed over less than the whole project. `requests-unapplied` names the narrowing requests the run could not apply, each with its reason in parentheses, for example `changed-since (git-failed)`. `baseline-path` carries the baseline the run compared against, as the `baseline` input spelled it, and stays empty for a baseline passed through `args`. GitLab writes the same names to `fallow-gates.env`, including `FALLOW_REQUESTS_UNAPPLIED`, and publishes that file through `artifacts:reports:dotenv`.

The sticky PR comment, the MR note, the inline review bodies and the Check Run carry the same verdict: the baseline advisory as a blockquote note, plus one row per armed gate with what it observed and the threshold it compared against. A gate the run did not enforce renders neutral, so the overall Check Run conclusion is unchanged. To make each gate its own required check, post the decision sidecar yourself with `fallow ci post-check-run --split-gates`, which creates one `Fallow / <gate>` status context per gate instead of a single `Fallow` check. Neither integration passes that flag.

In combined mode the CLI does not enforce the duplication threshold and says so in the envelope; the integrations honour that and warn rather than failing. Run the dedicated `dupes` command to gate on it.

On a fallow older than 3.27.0 the envelope carries no gate verdicts. Both integrations fall back to the fields those releases already published for the regression, security, baseline and semantic gates, and fail open with one warning for `threshold`, `min-score` and `min-severity`, which had no field to read. That warning appears only when the matching input is set.

On a fallow older than 3.28.0 the envelope carries no `request_outcomes`, and `baseline_staleness` carries no `scope_reasons` or `unrecognised_format`. `requests-unapplied`, `baseline-scope-reasons` and `baseline-unrecognised` stay empty there, the warnings that read them stay silent, and the whole-project baseline re-read falls back to reading the `production`, `workspace` and `changed-workspaces` inputs.

### Degraded and empty analysis

A run whose findings were computed over less than the whole project warns once, as `Fallow ran with degraded inputs:` followed by the diagnostic kinds and their counts. Both integrations select on `degrades_analysis` in `workspace_diagnostics`, so a diagnostic kind added in a later release is picked up as it is. A run that analyzed no source file at all gets its own sentence, because its clean result means nothing was measured rather than that nothing was found. It warns and passes by default; set `fail-on-empty-analysis` or `FALLOW_FAIL_ON_EMPTY_ANALYSIS` to fail instead.

A narrowing request the run could not apply warns as `Fallow could not apply:` followed by each request and its reason. The report then covers more of the project than was asked for, so do not read such a run as scoped to the change. The selector is `affects: "scope"`, so a request that writes a file beside the report never appears in this warning. See [`request_outcomes`](/cli/global-flags) for the request names, statuses and reasons. When `format: sarif` or `sarif: true` produced no SARIF document, the action warns separately that it uploads nothing and that code scanning keeps the alerts from the previous upload, with the reason from `request_outcomes["sarif-file"]` when the binary published one.

A narrowing request the run did apply, over a scope it measured as empty, warns the other way round, as `Fallow applied <request> over an empty scope`. No finding can survive a scope that measured nothing, so a clean report there covered nothing: check the diff or ref the run was given before reading it as a clean result. Both integrations key on `scope_size: 0` beside `status: "applied"` and on `affects: "scope"`, so a file written beside the report never triggers this warning. The rendered comment, review, job summary and annotation bodies carry the same fact as a clause on their request-outcome line.

A run that loads a baseline reports how much of that baseline still matches, and warns in the log and the job summary when entries have gone stale. Any run that loaded a baseline and was narrowed gets the baseline re-read over the whole project first, because a scoped run cannot judge a whole-project baseline. `baseline_staleness.scope_reasons` decides that: the integrations re-read when every channel that narrowed the run is one they added themselves (`diff`, `changed-since`, `changed-files`, `scope`, `file`, `issue-type-filter`), and they stand down and name the channel for `production`, `workspace` and `changed-workspaces`, which are your own statement about what the project is. That holds for scoping passed through `args` or `FALLOW_ARGS` too. The action publishes the list as `baseline-scope-reasons`. Set `fail-on-stale-baseline` (GitHub) or `FALLOW_FAIL_ON_STALE_BASELINE` (GitLab) to turn the staleness verdict into a failing build. Pointing the baseline and save-baseline inputs at the same file defeats it, because the run saves before it compares; the integrations warn when they see that. The PR comment and the MR note carry the gate line that `fallow report` renders, which states what each gate concluded.

A baseline the running command cannot read as its own (`baseline_staleness.unrecognised_format`) suppresses nothing. It is a baseline another command saved, or a file with nothing of this command in it. A baseline correctly saved on a project with nothing to record is not reported. `dead-code`, `dupes` and `health` all report it and all keep their exit code. The step log, the job summary, the PR comment, the MR note and the Check Run carry the sentence. The action publishes `baseline-unrecognised`, and the job summary names the file through `baseline-path`. The baseline gate trips on such a file, which `baseline-gate-trips` also reports, so `fail-on-stale-baseline` fails the job there. Baseline formats are per command; see [dead-code](/cli/dead-code), [dupes](/cli/dupes) and [health](/cli/health).

`command: audit` judges none of the baselines it loads, because it analyzes only the files that changed against its base. It prints one notice per loaded baseline, naming the entry count and the unscoped command that can judge it, and it rejects `--fail-on-stale-baseline` passed through `args` or `FALLOW_ARGS` with exit 2. See [audit](/cli/audit) for the per-analysis baseline inputs.

#### Where each advisory lands

A change-scoped run says nothing about staleness, because it cannot judge a whole-project baseline. The PR comment, the MR note and the Check Run render from that run's own envelope, so on a pull request they carry no baseline advisory, while the step log and the job summary carry the whole-project re-read's verdict. The baseline rows below describe a run that could judge what it loaded. The job summary and the Check Run are GitHub only; on GitLab read the job log, the MR note and the Code Quality report.
...
| Why the baseline could not be judged, and which channel narrowed the run | Yes | No | No | No |
...
| A narrowing request applied over an empty scope | Yes | Yes | Yes | Yes |
| SARIF requested and not produced (GitHub) | Yes | No | No | No |

The stood-down `stale-baseline` gate itself reaches every surface through the gate inventory. Only the sentence that says why, and which channel narrowed the run, stays in the log.
