...
The hook runs `fallow audit --format json --quiet --explain --gate-marker agent` and blocks the agent's `git commit` or `git push` only when the audit returns `verdict: "fail"`. The `--gate-marker agent` flag lets local [Impact](/analysis/impact) record blocked-then-cleared containment events when Impact tracking is enabled. Audit defaults to `gate=new-only`: inherited findings on touched files are reported in `attribution` and annotated with `introduced: false` on individual issues, but they do not affect the verdict. Only findings the changeset *introduces* fail the gate.
