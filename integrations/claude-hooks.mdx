...
Or override per invocation by editing `.claude/hooks/fallow-gate.sh` to pass `--gate all` explicitly. The script is regenerated on every `fallow setup-hooks` run, so prefer the config-file approach for persistence.
