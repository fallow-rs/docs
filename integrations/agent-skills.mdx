...
When the goal is to bring an existing codebase to a clean Fallow policy (not just audit a single PR), point the agent at the structured [adoption guide](/adoption) and use this prompt. The full version with context, reasoning, and decision tree lives on the adoption page; this shorter prompt is the one to paste into an agent.

````text
Adopt Fallow in this repository.

Read https://docs.fallow.tools/adoption and follow it end to end.

Constraints:
- Use full-repo analysis first (`fallow`, `fallow dead-code`, `fallow dupes`, `fallow health`), not `fallow audit`.
- Fix real issues before suppressing anything.
- Match each intentional exception to the narrowest correct mechanism:
  visibility tags, `publicPackages`, `entry`, `dynamicallyLoaded`, `ignorePatterns`,
  `ignoreDependencies`, `@expected-unused`, or a `rules`/`health` policy change.
- Only fall back to inline `fallow-ignore-*` comments for true one-offs.
- Turn on `fallow audit` as a PR gate only after the repo is clean.

Report at the end: code changes, config changes, exceptions added (with reasons),
anything left, and the final commands that demonstrate the repo is clean.
````

<Tip>
  The MCP server exposes `audit`, `dead_code`, `dupes`, and `health` as structured tools. Combine the adoption skill with [MCP](/integrations/mcp) so the agent gets JSON output with `_meta` explanations for every finding.
</Tip>
