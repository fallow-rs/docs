...
| `check_health` | Code complexity metrics and per-file health scores (`fallow health --format json`). Set `file_scores: true` for maintainability index per file. |
...
The MCP server is a thin subprocess wrapper. All analysis logic stays in the CLI binary. The MCP crate only handles protocol framing and argument mapping, built with `rmcp` (Rust MCP SDK). This means:
...
- `fallow-mcp` is available via `cargo install fallow-mcp` or from [GitHub Releases](https://github.com/fallow-rs/fallow/releases)
