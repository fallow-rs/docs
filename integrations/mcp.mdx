...
| `check_health` | Complexity metrics, file health scores, hotspots, and refactoring targets (`fallow health --format json`). Set `file_scores: true` for maintainability index, `hotspots: true` for churn analysis, `targets: true` for ranked recommendations sorted by efficiency, `trend: true` for per-metric deltas against the most recent snapshot. |
| `health_production_coverage` | Merge runtime production-coverage data into the health report (paid feature). Required `coverage` param accepts a V8 coverage directory, a single V8 coverage JSON, or an Istanbul `coverage-final.json`. Requires an active license JWT. Tunable via `min_invocations_hot`, `min_observation_volume`, `low_traffic_threshold`. Protocol-0.3+ sidecars emit a `summary.capture_quality` block flagging short-window captures. Can exceed the default 120s timeout on large dumps; raise `FALLOW_TIMEOUT_SECS` accordingly. |
...
| `analyze` | `group_by` | string | Group output by `owner` (CODEOWNERS) or `directory` (first path component) |
...
- **Health** (`check_health`, `audit`): `refactor-function` + suppress actions on findings, `apply-refactoring` + suppress on targets (when evidence exists), `refactor-file` + `add-tests` on hotspots.
