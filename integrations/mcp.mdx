...
| `get_token_blast_radius` | Tailwind v4 design-token blast radius from static analysis (free, no runtime dump). Runs `fallow health --css --format json`; read `css_analytics.token_consumers` for, per `@theme` token, its defining site plus a `consumer_count` and a capped located `consumers[]` sample of `{path,line,kind}` (`kind`: `theme-var`, `css-var`, `utility`, `apply`). A focused entry point so you do not need `css=true` on `check_health`. `consumer_count` is a static lower bound (a computed class name like `bg-${c}` is not counted), so use it to size the impact of editing or renaming a token, not to decide deletion; the dead-token verdict stays on `unused_theme_tokens`. |
