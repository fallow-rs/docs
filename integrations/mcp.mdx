...
Besides tools, the server exposes read-only reference material as MCP resources, so a client can list and cache them without spending a tool call. Every payload is JSON and carries `fallow_version`, so a cached copy is self-describing; the catalogue is fixed at build time (no subscriptions, no list-changed notifications).
...
Unknown URIs return a structured `resource_not_found` error listing the known URIs; an unknown issue type in the template lists the nearest matches. The same catalogue is published as `mcp_resources` in [`fallow schema`](/cli/schema).
