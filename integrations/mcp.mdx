...
The MCP server uses <Tooltip tip="Communication via standard input/output streams between parent and child processes">stdio transport</Tooltip> and a hybrid adapter. Supported requests run through the typed `fallow-api` in process, while command-owned options use a managed `fallow` CLI subprocess. Set `FALLOW_BIN` only when CLI-backed calls need a specific binary (defaults to `fallow` in `PATH`).
...
```mermaid
flowchart TB
  A["AI Agent<br/>Claude Code, Cursor, Windsurf"] -->|"typed tool calls<br/>via stdio"| B["MCP Server<br/>fallow-mcp"]
  B -->|"typed routes"| C["Rust API<br/>fallow-api<br/>in process"]
  B -->|"CLI-backed routes<br/>and fallbacks"| D["CLI Binary<br/>fallow<br/>managed subprocess"]
  C --> E["Results<br/>JSON output"]
  D --> E
```
