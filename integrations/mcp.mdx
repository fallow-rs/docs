...
The `save_baseline`, `save_regression_baseline`, and `save_snapshot` parameters write only inside the project root, its Git work tree, `RUNNER_TEMP`, or the system temp directory. Another path returns a tool error. See [Save destinations](/cli/global-flags#save-destinations).
