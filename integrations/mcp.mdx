...
description: "Integrate fallow with AI agents via CLI and MCP. Unused code, duplication, complexity hotspots, boundary violations, and auto-fix in Claude Code, Cursor, and Windsurf."
...
<Tabs>
  ...
  <Tab title="Cursor">
    Cursor reads MCP servers from `mcp.json`. Create `.cursor/mcp.json` in the project root to scope fallow to one project, or `~/.cursor/mcp.json` in your home directory to make it available everywhere:
...
| `find_dupes` | Code duplication detection (`fallow dupes --format json`). Each clone group carries a stable `fingerprint` and numeric `spread`; near-miss groups also carry `similarity`. Pass `near: true` to include function-scoped near-miss clones. Use the fingerprint with `trace_clone` to inspect a group. Import declarations are excluded by default; pass `ignore_imports: false` to count them again. `stats.clone_groups_ignored` reports config-filtered groups, while `stats.near_candidates_skipped` signals bounded near-miss comparisons that were skipped. Clone instances omit their verbatim source text by default; pass `include_fragments: true` to get it back. |
| `find_similar_code` | Read-only semantic function discovery through the pinned local model (`fallow similar-code --format json --quiet`). Returns unverified candidates with provider and model provenance, completion, limits, skips, and cache accounting. Scope with `files`, `workspace`, `changed_since`, `changed_workspaces`, `threshold`, `min_lines`, or `top`. The score is not a probability or verdict. The tool never downloads a model. If setup is missing, ask the user to run `fallow similar-code setup --local`. |
| `inspect_similar_code` | Reproduce one `candidate_id` and return bounded source, graph, ownership, churn, test, deterministic-duplication, and side-effect evidence. Read-only. Use it before judging a candidate and abstain when evidence is unavailable. The tool does not write verdicts or edit source. |
| `fix_preview` | Dry-run auto-fix preview (`fallow fix --dry-run --format json`) |
| `fix_apply` | Apply auto-fixes (`fallow fix --yes --format json`) |
| `guard` | Report the architecture rules that apply to given files before editing them: boundary zone, allowed import zones, forbidden calls, and rule-pack policies. |
...
| `get_cloud_runtime_context` | Runtime-context slice backed by fallow cloud instead of a local dump (`fallow coverage analyze --cloud --format json`), and the only fallow MCP tool that makes a network call. Required `repo` (`owner/repo`); `project_id`, `period_days` (1 through 90, default 30), `environment`, and `commit_sha` narrow the cloud selection, while `production`, `top`, and `min_invocations_hot` behave as on `check_runtime_coverage`. The key is `FALLOW_API_KEY` in the server environment and never a parameter: without one the call is refused before anything runs, with `code: "cloud_api_key_missing"`. Returns the same `runtime_coverage` block the local tools return, joined against the checkout at `root`, so a `root` on a different revision quietly empties `findings` and raises a `cloud_functions_unmatched` warning. Confirm `runtime_coverage.summary.data_source` is `cloud`, and read the source-map confidence table below before acting on file-level signals. |
...
| `feature_flags` | Detect feature flag patterns in the codebase ([`fallow flags --format json`](/cli/flags)). Identifies environment variable flags, SDK calls from common providers, and config object patterns. Set `top` to limit results. |
| `list_suppressions` | List active fallow-ignore suppression markers grouped per file with line, kind, level, reason, and a stale cross-reference ([`fallow suppressions --format json`](/cli/suppressions)). A read-only governance inventory for seeing what a clean verdict is hiding, not a gate: always exits 0 even when suppressions exist. Scope with `workspace`, `changed_since` (the natural pull-request review scope), or repeated `file` entries. Runs a full analysis, so raise `FALLOW_TIMEOUT_SECS` on large repos. |
...
| `trace_clone` | Deep-dive a duplicate-code clone group (`fallow dupes --trace <spec> --format json`). Address it by exactly one of: `file` + `line` (a source location), or `fingerprint` (a `dup:<id>` from a prior `find_dupes` result). Returns the matched instance plus every clone group containing it. Each traced group carries its `fingerprint`, an extract-function `suggestion`, and an optional `suggested_name`. Supports `mode`, `near`, `min_tokens`, `min_lines`, `threshold`, `skip_local`, `cross_language`, and `ignore_imports`. Use the same `near` value as the originating `find_dupes` call when tracing a near-miss group. |
| `trace_import_path` | Shortest import path between two modules (`fallow trace --path FROM TO --format json`). Required `from` and `to` file params. Returns `reachable`, `hops`, and a `path` array of import hops carrying `type_only` and `import_line`. An unreachable pair is a result, not an error: `reachable` is `false` with an empty `path`. The same module both ways answers `reachable: true`, and both cases report `hops: 0`, so branch on `reachable` rather than the count. Use it to see how one module ends up depending on another. |
| `trace_error` | Resolve a runtime stack trace's frames to the definitions they name (`fallow trace-error --format json`). Paste the trace text in the required `trace` param, with an optional `source` label. Classifies every frame as `in_project`, `node_modules`, or `out_of_corpus`, then resolves the in-project ones. A frame matching several definitions reports `ambiguous` and lists them; a frame matching nothing reports `not_found`; a frame never asked about reports `not_attempted`. No source maps are read, so a frame in generated build output is reported as such. See [`fallow trace-error`](/cli/trace-error). |
...
| `find_dupes` / `trace_clone` | `near` | bool | Include function-scoped near-miss clones with small structural edits. Default `false`. |
| `find_dupes` | `include_fragments` | bool | Include the verbatim source text on each clone instance. Default `false`: `file` plus the line and column range address the same code, and the text is most of the response. Set it `true` only when you need the source without reading the files. |
...
| `find_similar_code` / `inspect_similar_code` | `threshold` | number (0 to 1) | Model-specific discovery cutoff. It is not a probability or quality gate. |
| `find_similar_code` / `inspect_similar_code` | `min_lines` / `top` | integer | Exclude tiny functions and cap returned candidates. |
| `find_similar_code` / `inspect_similar_code` | `files` | string[] | Keep candidate pairs touching one of these root-relative files. |
...
| `audit` | `css_deep` | bool | Controls audit deep styling reachability. Omit it for the default project-wide styling pass, set `false` to skip project-wide styling reachability, or set `true` to force `--css-deep` when config disables it. |
| `audit` / `check_health` | `coverage` | string | Path to Istanbul-format `coverage-final.json` for accurate per-function CRAP scores. Resolution is consistent on typed and CLI-backed routes: an explicit tool parameter wins, followed by `FALLOW_COVERAGE`, then `health.coverage` from config. |
| `audit` / `check_health` | `coverage_root` | string | Absolute prefix to strip from file paths in coverage data before prepending the project root. Use when coverage was generated under a different checkout root in CI or Docker. Resolution is consistent on typed and CLI-backed routes: an explicit tool parameter wins, followed by `FALLOW_COVERAGE_ROOT`, then `health.coverageRoot` from config. |
...
CLI-backed tool calls run fallow with `--quiet`, so nothing the CLI would have written to stderr reaches the caller. What the run concluded about itself is restated instead as plain sentences on the response's root `warnings[]` array: a stale baseline, any gate that reported `fail` or `warn`, [diagnostics that degraded the analysis](/analysis/debugging), and any [request the run could not apply](/cli/global-flags#request-outcomes). A request that widened the report and an output file that was not written get separate sentences, because only the first means the findings cover more than you asked for. A narrowing request the run did apply over a scope it measured as empty gets its own sentence too: no finding survives a scope that measured nothing, so an empty result there is a report with nothing analyzable in it rather than clean code.

A baseline the command cannot read as its own gets its own sentence, ahead of the staleness sentence. Read `baseline_staleness.unrecognised_format` first, because such a file suppresses nothing: pass another path and do not re-save. That sentence names `--fail-on-stale-baseline` as a gate the run would fail, on every tool except `audit`, whose `stale-baseline` gate stands down for every baseline it loads.

The typed in-process route reads the same envelope members, so a tool answers the same way whichever route it took. Nothing is moved or removed to make room for the sentences: `gate_outcomes`, `baseline_staleness`, `workspace_diagnostics` and `request_outcomes` keep their own places in the envelope, a run with nothing to state adds nothing, and a warning never changes whether the call is an error.
...
|:---------|:------------|
| `FALLOW_BIN` | Path to the fallow CLI binary for CLI-backed calls. The MCP server checks, in order: this env var, a sibling binary next to `fallow-mcp`, then `fallow` in `PATH`. It does not affect typed in-process calls. |
| `FALLOW_TIMEOUT_SECS` | Deadline in seconds (default: `120`) for both typed and CLI-backed calls. A timed-out typed call returns `FALLOW_MCP_API_TIMEOUT`; a timed-out CLI call kills the process tree and returns `FALLOW_MCP_SUBPROCESS_TIMEOUT`. Increase for very large codebases. |
