...
Static analysis means building and traversing a graph, not reading files in a context window.
...
### Fix suggestions in tool responses

Every issue returned by the `analyze` and `check_changed` tools now includes an `actions` array with machine-actionable fix and suppress hints. The MCP server passes JSON through transparently from the CLI, so agents get the same structured actions described in the [dead-code CLI reference](/cli/check#fix-suggestions-in-json-output). Agents can use the `auto_fixable` flag to decide whether to call `fix_apply` or handle the suggestion manually.
...
<CardGroup cols={2}>
  ...
  <Card title="Dead code analysis" icon="skull-crossbones" href="/analysis/dead-code">
    Learn about the 15 issue types fallow detects.
  </Card>
</CardGroup>
