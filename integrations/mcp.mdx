...
CLI-backed tool calls run fallow with `--quiet`, so nothing that the CLI writes to stderr reaches the caller. The response states what the run concluded about itself, as plain sentences on the root `warnings[]` array: a stale baseline, any gate that reported `fail` or `warn`, [diagnostics that degraded the analysis](/analysis/debugging), and any [request the run could not apply](/cli/global-flags#request-outcomes).

A request that widened the report and an output file that the run did not write get separate sentences. Only the first one means the findings cover more than you asked for. A request that the run applied over a scope it measured as empty gets its own sentence. No finding survives a scope that measured nothing. An empty result there is a report with nothing analyzable in it, not clean code.

A baseline the command cannot read as its own gets its own sentence, before the staleness sentence. Read `baseline_staleness.unrecognised_format` first, because such a file suppresses nothing. Pass another path, and do not re-save. That sentence names `--fail-on-stale-baseline` as a gate the run would fail. It says so on every tool except `audit`, whose `stale-baseline` gate stands down for every baseline it loads.

The typed in-process route reads the same envelope members, so a tool answers the same way on either route. The sentences move and remove nothing. `gate_outcomes`, `baseline_staleness`, `workspace_diagnostics` and `request_outcomes` keep their own places in the envelope. A run with nothing to state adds nothing, and a warning never changes whether the call is an error.
