...
| `check_runtime_coverage` | (paid) Merge runtime runtime-coverage data into the health report. Required `coverage` param accepts a V8 coverage directory, a single V8 coverage JSON, or an Istanbul `coverage-final.json`. Requires an active license JWT. Tunable via `min_invocations_hot` (default 100), `min_observation_volume` (default 5000), `low_traffic_threshold` (default 0.001), `max_crap` (default 30.0), `top`, and `group_by`. Protocol-0.3+ sidecars emit a `summary.capture_quality` block flagging short-window captures. Can exceed the default 120s timeout on large dumps; raise `FALLOW_TIMEOUT_SECS` accordingly. Pick this over `check_health` when you have a coverage dump. |
| `get_hot_paths` | (paid) Runtime-context slice over the same local runtime coverage pipeline. Same input schema as `check_runtime_coverage`; read `runtime_coverage.hot_paths` for production hot paths sorted by percentile and invocation count. |
| `get_blast_radius` | (paid) Runtime-context slice for blast-radius review. Same input schema as `check_runtime_coverage`; until first-class `runtime_coverage.blast_radius` ships, combine `file_scores[].fan_in` with `runtime_coverage.hot_paths` and `runtime_coverage.findings`. |
| `get_importance` | (paid) Runtime-context slice for production-importance review. Same input schema as `check_runtime_coverage`; until first-class `runtime_coverage.importance` ships, combine `runtime_coverage.hot_paths`, `file_scores`, `hotspots`, and `targets`. |
| `get_cleanup_candidates` | (paid) Runtime-context slice for cleanup review. Same input schema as `check_runtime_coverage`; read `runtime_coverage.findings` for `safe_to_delete`, `review_required`, `low_traffic`, and `coverage_unavailable` verdicts. |
| `audit` | Audit changed files for dead code, complexity, and duplication ([`fallow audit --format json`](/cli/audit)). Returns a verdict (pass/warn/fail). Set `base` to specify the comparison ref (auto-detects if omitted). |
