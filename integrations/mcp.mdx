...
| `get_blast_radius` | Runtime-context slice for blast-radius review. Same input schema and free-vs-paid contract as `check_runtime_coverage`; until first-class `runtime_coverage.blast_radius` ships, combine `file_scores[].fan_in` with `runtime_coverage.hot_paths` and `runtime_coverage.findings`. |
| `get_importance` | Runtime-context slice for production-importance review. Same input schema and free-vs-paid contract as `check_runtime_coverage`; until first-class `runtime_coverage.importance` ships, combine `runtime_coverage.hot_paths`, `file_scores`, `hotspots`, and `targets`. |
...
| `audit` | Audit changed files for dead code, complexity, and duplication ([`fallow audit --format json`](/cli/audit)). Returns a verdict (pass/warn/fail). Set `base` to specify the comparison ref and `gate` to `new-only` or `all`. |
| `fallow_explain` | Explain one issue type without running analysis ([`fallow explain <issue-type> --format json`](/cli/explain)). Returns rationale, example, fix guidance, docs URL, and aliases. |
