...
<Accordion title="Extension settings">
  ...
  | `fallow.duplication.ignoreImports` | `true` | Exclude import declarations from duplicate-code detection. Enabled by default (sorted import blocks are a formatting artifact); set to `false` to count import blocks as clone candidates again. |
