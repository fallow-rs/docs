...
Commit `.vscode/settings.json` so the whole team sees the same scope. Pair the same tag with `fallow check --changed-since fallow-baseline` in CI to enforce the same contract on PRs.
