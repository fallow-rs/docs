...
keywords: ["static analysis", "runtime intelligence", "production coverage", "codebase intelligence", "hot paths", "cold paths", "dead code", "health"]
icon: "layer-group"
---
...
Production coverage is the collection mechanism. Runtime intelligence is what you get from it. The CLI surfaces everything through a single flag: `fallow health --production-coverage ./.fallow/runtime.json`. See the [production coverage analysis page](/analysis/production-coverage) for the collection setup.
...
Static intelligence is free and open source (MIT). You can run every static analysis on any project, today, with `npx fallow`. Runtime intelligence is the paid team layer, and `production coverage` is the collection engine that feeds it. Pricing, tiers, and what's included at each level live at [fallow.tools/pricing](https://fallow.tools/pricing).
