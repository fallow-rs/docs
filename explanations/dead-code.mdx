...
Fallow auto-detects 85 frameworks via `package.json`, so most convention-based exports (Next.js pages, Storybook stories, Jest configs) are handled out of the box. Run `fallow list` to see which plugins are active and what entry points they add.
