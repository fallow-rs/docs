...
A TypeScript namespace declared without the `export` keyword (`namespace Foo { export const inner = 1 }`, also `declare namespace` and the legacy `module Foo {}` form) is a local binding: its inner `export` declarations are members of that namespace, not exports of the file, so fallow never reports them here or under unused types. Only `export namespace Foo { ... }` contributes a file-level export (`Foo` itself).
...
A package in `devDependencies` that is imported by production source code via a runtime (value) import. This is the promote-side mirror of `type-only-dependencies` and `test-only-dependencies`: where those rules demote a production dependency that only tests or types need, this rule catches the opposite drift. A production-only install (`npm install --omit=dev`, `pnpm install --prod`, `yarn workspaces focus --production`) omits `devDependencies`, so the import breaks at runtime; the package belongs in `dependencies`.

| Severity | Default |
|:---------|:--------|
| Warning | Yes |

**What counts as a production import.** An import is production evidence only when all of the following hold:

- The importing file is reachable from a runtime entry point. Repo tooling that is not part of the shipped artifact (`scripts/`, benchmarks, playgrounds, files reachable only through a config-file chain such as a Rollup config importing helpers) does not count.
- The importing file is not a test, story, or fixture file (patterns like `*.test.*`, `*.spec.*`, `*.e2e.*`, `*.bench.*`, `*.stories.*`, `__tests__/`, `__mocks__/`, `test/`, `tests/`, the same exclusion set `--production` mode uses) and not a config file.
- The import is a runtime (value) import. A production file that imports the package only via `import type` does not trigger the rule, because type imports are erased at build time; this mirrors the import-kind analysis behind `type-only-dependencies`.

**What is never flagged.** A `devDependency` is skipped when another manifest section already provides it at runtime (it is also listed in `dependencies`, `peerDependencies`, or `optionalDependencies`), when it is a workspace package, when it is known dev tooling (`@types/*`, `typescript`, and similar), or when it is listed in `ignoreDependencies`. The rule currently checks the root `package.json` against the project-wide import graph; imports from files owned by a workspace package are skipped because the workspace's own manifest governs their runtime resolution (per-workspace detection is a shared follow-up with the sibling dependency rules).

**Interaction with `--production` mode.** Unlike `test-only-dependencies`, which needs test files in the graph to answer its question, this rule stays on in `--production` mode. Production mode only makes the signal cleaner: every discovered file is production code, and a production CI run is exactly where a prod-only install breakage matters.

**Known limitation.** A file referenced from a `package.json` `scripts` command counts as a runtime entry point, so a dev-only helper script that value-imports a `devDependency` can still be flagged. Suppress such a package via `ignoreDependencies`.
...
There is no inline suppression comment for this rule (`suppressible: false` in the capability manifest). That is intentional: the finding anchors on the package's line in `package.json`, and JSON cannot carry comments. To silence a specific package, add it to `ignoreDependencies`; to silence the rule entirely, set it to `off`.

**When to act:** Move the package from `devDependencies` to `dependencies`. If the flagged import is genuinely dev-only tooling (the script-entry limitation above), add the package to `ignoreDependencies` instead.
...
`unused-dependency-overrides` reports override entries whose target package is not declared by any workspace `package.json` and is not present in the lockfile (`pnpm-lock.yaml`, `package-lock.json`, `npm-shrinkwrap.json`, or bun's text `bun.lock`). Overrides that target transitive dependencies resolved in the lockfile (the common CVE-pin pattern) are treated as used. When no lockfile is present, fallow falls back to a package.json-only check and keeps a `hint` on each finding so transitive pins can be reviewed before removal. When the only lockfile next to the root `package.json` is bun's legacy binary `bun.lockb` (no parseable text lockfile beside it; a `yarn.lock` is never consulted), resolution ground truth is unreadable, so the check is skipped rather than degraded, and fallow says so through the `bun-lockb-override-resolution-skipped` workspace diagnostic, which names the lockfiles it looked for and hints `bun install --save-text-lockfile` (bun 1.2 or newer) or deleting a stale `bun.lockb`. `misconfigured-dependency-overrides` reports malformed override keys or empty values that pnpm rejects.
