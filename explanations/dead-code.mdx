...
Fallow detects 14 types of dead code. Each type has a default severity that determines whether it causes a non-zero exit code with `--fail-on-issues`.
...
8. **Test-only dependencies** — install size optimization
9. **Duplicate exports** — developer experience
10. **Unused types / enum members / class members** — code hygiene
