...
`unused-dependency-overrides` reports override entries that force a version for a package no workspace depends on directly. `misconfigured-dependency-overrides` reports malformed override keys or empty values that pnpm rejects.
...
Fallow auto-detects 94 frameworks via `package.json`, so most convention-based exports (Next.js pages, Storybook stories, Jest configs, tap suites, and tsd declaration tests) are handled out of the box. Run `fallow list` to see which plugins are active and what entry points they add.
