...
- **Type-3 clones** (gapped clones with inserted/deleted lines) are not detected by default. Add `--near` to find function-scoped near-miss clones with small structural edits. Near-miss detection uses semantic normalization and requires at least 80% shingle similarity between every pair in a group.
...
This is an excerpt. `_meta` carries an entry for every metric the report can emit, plus a `field_definitions` block describing shared wire fields such as `actions[]`.
