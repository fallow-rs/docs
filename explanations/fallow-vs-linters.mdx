...
<Info>
`eslint-plugin-import` builds a cross-file graph for `no-unused-modules` and `no-cycle`, but reconstructs it from scratch on every lint run with no caching. Fallow builds and caches the module graph once, resolves barrel file re-export chains, and runs all analyses in milliseconds rather than minutes. Where the tools overlap, the capability is the same; the performance difference is 100-1000x.
</Info>
