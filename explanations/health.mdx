...
<Tip>
**Quick start**: Run `fallow health --targets` to get a prioritized refactoring list. Filter by `effort: low` for quick wins. Save a baseline with `--save-baseline`, add `--baseline` to CI, and prevent new tech debt from landing. Update the baseline as you fix targets.
</Tip>
