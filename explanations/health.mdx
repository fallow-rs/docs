...
|:----------------|:-------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<template>`    | The `.html` file or the `.ts` file containing the inline literal         | Template control-flow blocks (`@if`, `@for`, `@switch`, `@defer`, `@let`, legacy `*ngIf`/`*ngFor`), bound attributes, and interpolations                                                      |
| `<component>`   | The `.ts` file declaring the `@Component`                                | `worst_class_method + template` for each metric. Anchored at the worst class method's line. Ranking and `--targets` use this rollup; the `component_rollup` payload carries the per-half breakdown |
