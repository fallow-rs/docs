...
description: "What each metric in fallow's health output means, how it's computed, and how to interpret the values. Complexity analysis, hotspot detection, and refactoring targets integrated with dead-code and duplication findings."
...
Project-wide cyclomatic averages, percentiles, and critical shares include authored functions, module-scope branching, and template units. `vital_signs.cyclomatic_population` reports `count`, `sum`, and `max` for each group. Add the sums and divide by the total count to reconstruct the mean, rounded to one decimal place. An empty group has a `null` maximum; older saved reports omit the population field.

Module-scope units contribute to these aggregates without producing function findings. `summary.functions_analyzed` counts functions and template units checked for findings, excluding module scopes. For example, one function with complexity 1 and a module scope with complexity 31 produce an average of 16 even when the function has no finding. The population fields make that difference explicit.

Compact output appends population rows after the unchanged `vital-signs` line, in functions, modules, then templates order. For the example above:
...
Older reports or results without population data omit all three rows.
...
Angular components render their work across two surfaces: a TypeScript class (methods, getters, lifecycle hooks) and a template (HTML control flow, bindings, interpolations). Fallow flags both, but the per-half findings would understate the total complexity an agent or developer is responsible for. Two synthetic finding shapes close that gap:
...
| `<component>`   | The `.ts` file declaring the `@Component`                                | Worst class method's score plus the template's score, for each metric. Ranking and `--targets` use the rollup totals; the per-half breakdown lives under `component_rollup` (`class_cyclomatic`, `class_cognitive`, `template_cyclomatic`, `template_cognitive`) |

Both shapes route to the same rule ids as function findings: `fallow/high-cyclomatic-complexity`, `fallow/high-cognitive-complexity`, and `fallow/high-complexity`. They use the same thresholds (`max_cyclomatic`, `max_cognitive`) and the same severity bands. Templates that exceed the configured threshold are surfaced individually; if both a `<template>` and at least one class method on the same component file exceed thresholds, a `<component>` rollup is added alongside them.

Acting on these findings differs from function findings:

- **`<template>` findings** are easiest to address by splitting into child components, hoisting derived data into the class as computed signals (`computed(...)`), or replacing nested control flow with a flatter structure.
- **`<component>` rollup findings** point to a component whose two halves together exceed the budget. Attack the larger half first: the `component_rollup.class_*` and `component_rollup.template_*` fields show which side contributes more.

CRAP scoring on `<template>` findings inherits from the owning component class: the `templateUrl` edge means a template is reachable iff its class is reachable, and tests that exercise the class also exercise the template. The `inherited_from` field on a `<template>` finding's payload names the `.component.ts` file whose coverage drove the score, so the `increase-coverage` action targets the testable surface (the class) rather than the structurally untestable `.html`.
...
The 60-LOC threshold is configurable via `health.maxUnitSize` (globally) or a per-file `health.thresholdOverrides[].maxUnitSize` entry. Raising it for a glob (e.g. `**/*.test.*`) removes those functions from the **Large functions** list while keeping complexity, cognitive, and CRAP checks on the same files, which is the recommended way to stop test suites (long `describe()` / `it()` blocks) from dominating the list. Like the other overrides, `maxUnitSize` filters the reported list; the descriptive `unit_size_profile` and the unit-size score still reflect raw sizes, so use `health.ignore` if you want a file out of the score entirely.
...
The `add_test_coverage` refactoring target (label `untested risk`) fires when both conditions are true:
...
The default `static_estimated` model is an approximation of test coverage:
...
Effort uses OR-semantics for "high" (any single criterion triggers it) and AND-semantics for "low" (all three must be true). The fan-in percentiles are computed from the project's file-score distribution (P25 floor 2, P95 floor 5), so a small file whose fan-in sits at or above the project's P95 is still "high" because changes ripple widely.

In human output, effort appears after the category label separated by ` · `:
```text
...
| All other categories | `evidence` omitted from JSON | — |
...
`health_score.formula_version` identifies the formula used. Version 2 uses scale-invariant density and tail metrics so large monorepos are not diluted by many healthy files. Older snapshots that lack the v2 fields fall back to the previous average, p90, and raw-count aggregators.
...
An input that ran but failed to load is a separate case: it contributes zeros, which reads like a clean measurement. Check `workspace_diagnostics[]` for an entry with `degrades_analysis: true` before taking a zero at face value. See [degraded inputs](/cli/health#degraded-inputs).
