...
An input that failed to load is a different case. It contributes zeros, and a zero reads like a clean measurement. Before you trust a zero, check `workspace_diagnostics[]` for an entry with `degrades_analysis: true`. See [degraded inputs](/cli/health#degraded-inputs).
