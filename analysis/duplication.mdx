...
Clone group #1 (3 instances, 12 lines each)
  src/api/users.ts:15-26
  src/api/posts.ts:22-33
  src/api/comments.ts:18-29
  → Extract function: shared fetch-and-transform pattern

Clone group #2 (2 instances, 8 lines each)
  src/components/UserCard.tsx:42-49
  src/components/ProfileCard.tsx:38-45
  → Extract function: same file set, consider shared component

Clone family: src/api/*.ts
  3 clone groups share the same file set
  → Consider extracting a shared API utility module

Duplication: 3.2% (127 duplicated lines / 3,891 total)
Found 5 clone groups in 2 clone families (45ms)
```

In semantic mode, fallow also reports renamed identifiers:

```bash title="$ fallow dupes --mode semantic"
Clone group #1 (2 instances, 15 lines) [semantic match]
  src/services/userService.ts:10-24
  src/services/orderService.ts:8-22
  Renamed: userData→orderData, userId→orderId, fetchUser→fetchOrder
  → Extract generic service pattern

Duplication: 1.8% (semantic mode)
Found 3 clone groups (62ms)
```
