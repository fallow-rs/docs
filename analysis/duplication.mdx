...
description: "Detect copy-pasted code blocks across your TypeScript and JavaScript codebase. Built-in to the same binary as dead-code analysis. Suffix-array detection runs 8-26x faster than standalone tools."
...
Files with the same set of imports (sorted alphabetically by a formatter) are a structural property of well-formatted code, not copy-paste, so `import` declarations are stripped from the token stream by default. This covers ES `import` declarations only; CommonJS `require()` calls and `export ... from` re-export blocks are still counted.

To count import declarations as clone candidates again, opt out on the command line:
