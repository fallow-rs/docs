...
<Tabs>
  <Tab title="Bulletproof">
    ...
    **4 logical zones:** `app`, `features`, `shared`, `server`. The `features` zone auto-discovers immediate child directories so sibling features are isolated as `features/<name>` zones.
