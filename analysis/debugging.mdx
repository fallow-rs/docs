...
When a health input does not load, the section that needed it reports nothing
instead of zero, and the run records why. Every entry carries a `message` ending
in a next step, and is recorded whether or not `--quiet` is passed. Select on
`degrades_analysis` rather than on a list of kinds:
...
|:-------|:--------------|:------------------|
| `file-scores-unavailable` | Per-file scoring failed, so the score list is empty and every score-derived number is structurally zero | Yes |
| `hotspots-skipped` | The hotspot, churn and ownership sections report nothing. `cause` names which input stopped them: `not-a-repository`, `invalid-since` or `churn-file-unreadable` | Yes |
| `shallow-clone` | Churn covers the fetched history only. `ownership_requested` says whether ownership was asked for, and therefore skewed too | Yes |
...
| `ownership-unavailable` | Ownership inputs did not load, so owner signals are degraded or absent. `cause` is `invalid-bot-pattern` or `codeowners-parse-failed` | Yes |
| `trend-snapshot-unreadable` | A saved snapshot the trend could not read, so the direction is computed over fewer points than the project has on disk | Yes |
| `coverage-auto-detected` | Provenance, not a failure: coverage was found on disk rather than passed with `--coverage`, and `path` names the file that fed the CRAP scores | No, the field is absent |
...
| `plugin-effect-not-modeled` | A framework plugin read a config key whose effect fallow does not model, so a convention stayed in place. Same payload, with `reason` `key-effect-not-modeled` or `config-property-unreadable` | No, the field is absent |

The two plugin kinds come from a config rather than from a health input, so any
run with a dead-code pass carries them: `dead-code` / `check`, `health`, `flags`,
`audit` and the bare combined run. A standalone `fallow dupes` carries neither.
The `reason` token sets are open as well. See
[Module Federation](/frameworks/built-in#module-federation) for the keys that
report one.

**Fix**: read the entry's `message`. Which sections report nothing, and what a
section reports when it measured zero instead, is covered in
[`fallow health`](/cli/health). The kind set is open, so treat a kind you do not
recognize as one more diagnostic rather than as an error.
