...
Fallow prints every file that imports `formatCurrency`, including indirect imports through barrel re-exports. Use it when:
...
Fallow shows every import in the file and every file that imports from it. Use it when:
...
To find every place that uses a package, pass `--trace-dependency PACKAGE`. This covers imports, script binaries, and plugin detection.
...
Fallow reports all import sites, the `package.json` scripts that use a binary of the package, and the plugins that detect it. Use it when:
...
The `(other)` row is the time inside `TOTAL` that belongs to no named stage, such as report assembly and the steps between stages. The sequential stages plus `(other)` add up to `TOTAL`.
...
By default, fallow does not analyze packages in `peerDependencies` or `optionalDependencies`, because the project that installs your package can supply them.

**Fix**: If fallow reports these packages by mistake, add them to `ignoreDependencies`:
...
**Fix**: No config field adds a hidden directory to discovery. Analyze the
directory on its own with `fallow dead-code --root .claude`. If it holds tool
state that you never want to analyze, add it to `ignorePatterns`. For the full
behavior, see [Known limitations](/analysis/limitations).
