...
description: "Trace findings, diagnose false positives, and understand fallow's analysis pipeline."
keywords: ["debugging", "troubleshooting", "false positives", "trace", "performance", "cache"]
icon: "bug"
---

When fallow reports something unexpected, its built-in tracing and performance tools help you understand *why*. This page covers the debugging workflow for each type of finding.
...
```bash
fallow check --trace src/utils/format.ts:formatDate
```
...
This prints every file that imports `formatDate`, including indirect usage through barrel re-exports. Use this when:
...
Use `--trace-dependency PACKAGE` to find everywhere a package is used -- through imports, script binaries, and plugin detection. This answers: "Where is this dependency actually consumed?"
...
The output shows timing for each stage: config loading, file discovery, parsing (with cache hit/miss statistics), resolution, graph construction, and analysis. Use this when:
