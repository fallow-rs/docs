...
instead of zero, and the run records why. Every entry carries a `message` that
ends with a next step. Fallow records the entry with or without `--quiet`. Select
on `degrades_analysis` rather than on a list of kinds:
...
**Fix**: read the entry's `message`. [`fallow health`](/cli/health) tells you
which sections report nothing, and what a section reports when it measured zero.
The kind set is open, so read a kind you do not recognize as one more diagnostic
