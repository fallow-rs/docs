...
- **`add-to-config` for `ignoreExports`** (duplicate-exports): `true` when a fallow config file exists on disk; `false` otherwise. The applier declines to materialize a config file from scratch.
