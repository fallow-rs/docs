...
Fallow scans infrastructure config files for source file references and treats them as entry points. This prevents worker processes, migration scripts, and other infrastructure-defined processes from being reported as unused files.
...
Barrel files (`index.ts` files that re-export from other modules) are common in TypeScript projects. Fallow fully resolves `export *` chains through multiple levels of barrel files with cycle detection.
