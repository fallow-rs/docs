...
description: "Find unused files, exports, dependencies, types, class members, circular dependencies, and boundary violations in TypeScript and JavaScript. Inheritance-aware class member detection, CSS/SCSS tracking, and cycle detection built into the module graph."
...
When two `export *` sources contribute the same name, ECMAScript treats that
...
[`fallow trace FILE:NAME`](/cli/trace) to inspect the contributing source files
...
See [CSS, SCSS, and Tailwind analysis](/analysis/css-analysis) for full details.
