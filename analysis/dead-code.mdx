...
```mermaid
flowchart TB
  A["Discovery<br/>Entry points from package.json + plugins"] --> B["Parsing<br/>Parallel with Oxc + rayon"]
...
<Info>
  9-34x faster than madge, 3-12x faster than dpdm. Fallow uses 5-7x less memory because it reuses the module graph already built for dead code analysis.
</Info>
