...
<Tabs>
  ...
  <Tab title="JSX templates">
    SSR frameworks like [Hono](https://hono.dev) render HTML by returning JSX from layout components. Fallow treats `<script src="...">` and `<link rel="stylesheet|modulepreload" href="...">` inside JSX/TSX files the same way it treats them in plain HTML: as asset references that keep the target file reachable.

    **What's supported:**

    - `<script src="./app.js">` inside JSX/TSX layouts
    - `<link rel="stylesheet" href="./global.css">` inside JSX/TSX layouts
    - `<link rel="modulepreload" href="./vendor.js">` inside JSX/TSX layouts
    ...
    Fallow sees this file as referencing `/static/style.css` and `/static/app.js`, so sibling files in `static/` are correctly marked reachable.

    <Note>
    Only lowercase intrinsic elements (`<script>`, `<link>`) and plain string literals (`src="foo.css"`) are tracked. Capitalized React-style components (`<Script>`, `<Link>`) and expression containers (`href={someVar}`) are intentionally skipped because they have component-specific semantics that fallow can't statically resolve.
    </Note>
  </Tab>
  <Tab title="JSDoc">
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in plugins" icon="puzzle-piece" href="/frameworks/built-in">
    85 framework plugins for automatic entry point detection.
  </Card>
</CardGroup>
