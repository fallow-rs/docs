...
<Tabs>
  <Tab title="HTML">
    ...
    **Angular template complexity.** Standalone `.html` files that use Angular template syntax also contribute synthetic `<template>` complexity findings to `fallow health`. The scanner recognises control-flow blocks (`@if`, `@for`, `@switch`, `@case`, `@defer (when ...)`, `@let`), legacy structural directives (`*ngIf`, `*ngFor`), bound attributes (`[x]`, `(x)`, `bind-x`, `on-x`), and `{{ }}` interpolations. Suppress with `<!-- fallow-ignore-file complexity -->` at the top of the template.
  </Tab>
