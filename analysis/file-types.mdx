...
Fallow analyzes more than `.js` and `.ts` files. It extracts imports and exports from a range of non-JavaScript file types, so your module graph stays complete without manual configuration.

<Info>
All file type support is built in. Fallow detects these files automatically based on their extension -- no plugins or config needed.
</Info>
