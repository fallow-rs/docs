...
<Tabs>
  ...
  <Tab title="SSR layouts">
    ...
    **What's supported:**
    ...
    In tagged templates, only the bare identifier tag `html` is matched — `css`, `sql`, `gql`, and similar tags are left alone. Asset references split across an interpolation boundary (`` html`<script src="${base}/app.js">` ``) are skipped rather than producing garbled specifiers.
    </Note>
  </Tab>
  <Tab title="JSDoc">
