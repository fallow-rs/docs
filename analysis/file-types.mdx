...
description: "Fallow analyzes HTML, Vue, Svelte, Astro, MDX, GraphQL, CSS modules, SCSS, and React Native files automatically. No plugins or config needed."
...
<Tabs>
  <Tab title="HTML">
    Fallow discovers `.html` files and extracts edges from `<script src>` and `<link>` tags. This covers projects that use HTML entry points (such as Parcel or vanilla setups).
