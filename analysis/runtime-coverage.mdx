...
| `findings` | Cold or unresolved functions with a per-finding suppression `id` (`fallow:prod:<hash>`, hashes the current line), an optional cross-surface `stable_id` (`fallow:fn:<hash>`, hashes file + name + start line so it survives line moves), `line`, per-finding `verdict`, `confidence`, supporting `evidence`, and machine-readable `actions`. |
