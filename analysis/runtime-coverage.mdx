...
`test_only_reference` guards the one case where production mode and static
reachability disagree. Production mode drops test, spec, story, fixture, and
benchmark files from discovery, so an export whose only callers live in those
files reads as statically unused. It is `true` when the function is unreachable
in the production graph but still referenced from an excluded file, which means
it is not dead code: removing it breaks the referencing test. Such a function is
reported as `review_required` rather than `safe_to_delete`, with the action
"Only tests reference this export; delete the test usage together with the
function or keep it". It is `false` when the production graph was compared
against the full tree and no such reference exists, and `null` when the report
was produced without a production filter, so there is no second reachability
answer to report.
