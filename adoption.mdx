...
<Steps>
  ...
  <Step title="Clean up unused exports, types, enum members, and class members">
    ...
    The same command reports unused enum members and class members alongside exports and types.
  </Step>
...
<Tip>
  `@expected-unused` is self-cleaning. Unlike inline `fallow-ignore` comments, Fallow tracks the tag for staleness: the moment the export becomes imported, the tag is reported as a stale suppression so you can remove it. No other tool in the TypeScript dead-code ecosystem tracks suppression staleness; use this tag liberally for shims and future API surface.
</Tip>
...
- `fallow audit` runs cleanly on the default branch
...
<Tabs>
  <Tab title="Option A: warn-everywhere">
    Best for teams under ~20 developers. CI never blocks; fixes land under social pressure.
  ...
  <Tab title="Option B: error + baselines">
    Best for larger orgs where warning-only gates are ignored. CI blocks on new issues; pre-existing debt on touched files is baselined.
    ...
    Commit the baseline files to the repo so every PR compares against the same snapshot. Regenerate on a schedule (quarterly, or per release) rather than per merge, otherwise teams silently absorb new debt every time CI runs.
  </Tab>
</Tabs>
...
<CardGroup cols={3}>
  <Card title="Skills (best)" icon="wand-magic-sparkles" href="/integrations/agent-skills">
    Install `fallow-skills` for Claude Code, Cursor, Windsurf, or any Agent Skills compatible agent. Works offline; the agent gets full workflow knowledge without fetching URLs.
  </Card>
