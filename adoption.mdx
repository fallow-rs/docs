...
The [quick start](/quickstart) covers the focused commands you will use during cleanup (`dead-code`, `dupes`, `health`) and `fallow init` for generating a starting config.
...
<Steps>
  ...
  <Step title="Clean up unused exports, types, enum members, and class members">
    Usually the biggest category. Fix real dead code. Model intentional API surface with visibility tags or config instead of suppressing it. Hand this step to an agent using the [prompt in section 8](#8-clean-up-with-an-agent).
