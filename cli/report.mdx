...
A re-render also restates the verdicts the saved envelope carries, so a rendered
body says the same thing as the run that produced it. The PR-comment and review
bodies reproduce the baseline advisory, the gate verdicts and the
[request outcomes](/cli/global-flags#request-outcomes); `github-summary` and
`github-annotations` reproduce the gate verdicts and the request outcomes. One
exception: the comment and review targets state a diff filter that stood down in
the rendering process itself, because the diff those bodies are filtered by is
the one you hand `fallow report`, not the one the analysis run used.

A bare combined run is the one exception to that parity. Its live `pr-comment-github`, `pr-comment-gitlab` and review bodies carry a multi-gate presentation and no status note. `fallow report --from` renders the generic body for a saved combined envelope, and that body carries the note. Both shipped CI integrations render through `fallow report --from`, so their comments carry it.
