...
says what the run said. The PR-comment and review bodies repeat the baseline
advisory, the gate verdicts and the
[request outcomes](/cli/global-flags#request-outcomes). `github-summary` and
`github-annotations` repeat the gate verdicts and the request outcomes.

The comment and review targets are different in one point. They report a diff
filter that did not apply during the re-render. Those bodies use the diff you
give `fallow report`, not the diff of the analysis run.

A bare combined run is the one exception. Its live `pr-comment-github`, `pr-comment-gitlab` and review bodies carry a multi-gate presentation and no status note. `fallow report --from` renders the generic body for a saved combined envelope, and that body carries the note. Both shipped CI integrations render through `fallow report --from`, so their comments carry the note.
