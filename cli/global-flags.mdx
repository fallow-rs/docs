...
The key set, the `status` values, and the `affects` values are open. Read a name, a status, or a class you do not recognize as "some request" and "some outcome" rather than as an error, and never as `applied` or as `scope`. Select on `affects` instead of on a list of names: a request added later carries its own class. A diff that parsed but names no analyzable file reports `applied`, because the filter was applied over an empty scope.

Every unapplied request also warns on stderr. The entry in the envelope is recorded whether or not `--quiet` is passed, so it is the channel a machine consumer can rely on. [`fallow audit`](/cli/audit) carries no object: it exits 2 rather than widen when its `--base` will not resolve. [`fallow security`](/cli/security) records no `sarif-file` entry, because a failed write there exits 2 instead of continuing.
...
The GitHub Action and the GitLab template also publish the unapplied narrowing requests as a job output and a dotenv variable. See [CI integration](/integrations/ci).
