...
| `--fail-on-stale-baseline` | Exit 1 when a loaded `--baseline` has any entry that matched nothing this run, on `dead-code` / `check`, the bare run, `dupes` and `health`, in every output format. The verdict is the exit code plus one stderr line naming the stale count, the baseline path and the re-save command. A run that cannot judge the baseline (a narrowed scope, `health --report-only`, `audit`, `decision-surface`) stands down and says so on stderr instead of passing quietly. Report output is unchanged: the verdict is published as `baseline_staleness.gate_trips` whether or not this flag is passed. That member describes the baseline, not the exit code, so a stood-down run still reports it as true. |
