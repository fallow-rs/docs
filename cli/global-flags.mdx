...
| `--dupes-near` | Include function-scoped near-miss clones in combined mode. Mirrors the standalone [`fallow dupes --near`](/cli/dupes). |
...
| `--dupes-ignore-imports` | Exclude module wiring from duplicate detection in combined mode (default). Falls back to the config value when unset. Mirrors the standalone [`fallow dupes --ignore-imports`](/cli/dupes). |
| `--dupes-no-ignore-imports` | Count module wiring as clone candidates in combined mode (opt out of the default exclusion). Conflicts with `--dupes-ignore-imports`. Mirrors the standalone [`fallow dupes --no-ignore-imports`](/cli/dupes). |
