...
| `--fail-on-stale-baseline` | Exit 1 when a loaded `--baseline` has any entry that matched nothing this run, on `dead-code` / `check`, the bare run, `dupes` and `health`, in every output format. The flag also exits 1 when the loaded file is a baseline of another `kind`, because such a file suppresses nothing. The verdict is the exit code plus one stderr line naming the stale count, the baseline path and the re-save command. A run that cannot judge the baseline (a narrowed scope, `health --report-only`, `audit`, `decision-surface`) stands down and says so on stderr instead of passing quietly. A narrowed scope stands down for the unmatched-entry half only: which command saved a file does not depend on how much of the project the run covered. Report output is unchanged: the verdict is published as `baseline_staleness.gate_trips` whether or not this flag is passed. That member describes the baseline, not the exit code, so a stood-down run still reports it as true. `gate_outcomes["stale-baseline"].status` reports `fail` for both halves of the rule, and stays `skipped` on `audit`. |
...
| `--group-by <MODE>` | Group output by CODEOWNERS ownership (`owner`), first directory component (`directory`), workspace package (`package`), or GitLab CODEOWNERS section (`section`). Carried by `json`, `human`, `sarif` and `codeclimate`; every other format renders one flat document and says so. See [Grouped output](#grouped-output). |
...
`--changed-since`, a supplied diff and `--sarif-file` ask a run to narrow its report or to write a file beside it. When one of them cannot be honored, the run continues and the exit code does not move. `--format json` records what became of every request it received in a root object keyed by request name, the same shape `gate_outcomes` uses for what the run concluded:
...
| Member | Value |
|:-------|:------|
| `status` | `applied` when the run did what it was asked, `not-applied` when it could not and continued anyway. |
| `affects` | `scope` when the request narrows the report, `artifact` when it writes a file beside the report. Read this before deciding what an unapplied entry cost: on `scope` the report below is complete, valid, and wider than you asked for; on `artifact` a file was not written and the report's own scope is untouched. |
| `requested` | What you asked for, as you spelled it: the git ref, the diff source label, the `--sarif-file` path. Echoed verbatim, so do not join it to the project root the way you join every other path-shaped field. |
| `scope_size` | How much the request left in scope, present only when the run applied it and measured that scope. The unit belongs to the request name: `diff-filter` counts added lines, so read the unit off the key the entry sits under, never across names. The count is what the run indexed, capped at one million added lines, so treat a non-zero value as a lower bound. `0` means the narrowing left nothing to analyze, so a report with no findings covered nothing. Absent means not measured, never zero. |
| `reason` | A kebab-case token, present exactly when `status` is not `applied`. |
| `message` | One sentence naming what happened and the next step, present exactly when `status` is not `applied`. The same sentence the run writes to stderr. |

Honored requests are listed too, so an absent `request_outcomes` means nothing was asked for, never that nothing failed.
...
The key set, the `status` values, and the `affects` values are open. Read a name, a status, or a class you do not recognize as "some request" and "some outcome" rather than as an error, and never as `applied` or as `scope`. Select on `affects` instead of on a list of names: a request added later carries its own class. A diff that parsed but names no analyzable file reports `applied` with `scope_size: 0`, because the filter was applied over an empty scope.

Every unapplied request also warns on stderr. The entry in the envelope is recorded whether or not `--quiet` is passed, so it is the channel a machine consumer can rely on. [`fallow audit`](/cli/audit) carries no object: it exits 2 rather than widen when its `--base` will not resolve. [`fallow security`](/cli/security) records `sarif-file` as `applied` with the path it wrote, so you can tell a run that produced the document from one that was never asked for it; a failed write there exits 2 with an error document instead of continuing, so no envelope carries that entry unapplied. [`fallow flags`](/cli/flags) and [`fallow suppressions`](/cli/suppressions) carry the `changed-since` entry and nothing else, because neither applies a diff filter.
...
When a narrowing request applied over an empty scope, the same line adds a clause saying the report is clean because nothing in it was analyzable, and to check the diff or ref the run was given.

The GitHub Action and the GitLab template also publish the unapplied narrowing requests as a job output and a dotenv variable, and both warn on an empty scope. See [CI integration](/integrations/ci).
...
| `compact`, `markdown`, `badge`, `pr-comment-github`, `pr-comment-gitlab`, `review-github`, `review-gitlab`, `github-annotations`, `github-summary` | One flat document, plus a stderr note naming the format you passed. The exit code does not change. The four comment and review bodies also carry a clause with the mode you requested, pointing at `--format json`. |
