...
| `--dupes-ignore-imports` | Exclude import declarations from duplicate detection in combined mode (default). Falls back to the config value when unset. Mirrors the standalone [`fallow dupes --ignore-imports`](/cli/dupes). |
| `--dupes-no-ignore-imports` | Count import declarations as clone candidates in combined mode (opt out of the default import exclusion). Conflicts with `--dupes-ignore-imports`. Mirrors the standalone [`fallow dupes --no-ignore-imports`](/cli/dupes). |
