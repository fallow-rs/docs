...
Impact is **opt-in and local-only**. History lives in your user config dir (see [Storage and cleanup](#storage-and-cleanup)): **nothing is ever written into the repo** (no `.fallow/` directory, no `.gitignore` change), nothing is uploaded, and recording never affects exit codes or analysis output. Recording is also skipped in CI.
...
fallow impact reset         # delete this project's history
...
Each project keeps one small history file in your user config dir (`~/Library/Application Support/fallow/` on macOS, `$XDG_CONFIG_HOME/fallow/` on Linux, `%APPDATA%\fallow\` on Windows), keyed by repository, so running `fallow impact` from any subdirectory or git worktree of a repo shows that repo's history. Concurrent recordings (for example, a pre-commit gate firing in two worktrees at once) are serialized with an advisory lock so neither run loses the other's data.
...
<CardGroup cols={3}>
  <Card title="fallow init" icon="terminal" href="/cli/init">
    Install the pre-commit hook that passes the gate marker Impact records.
  </Card>
  <Card title="Claude Code hooks" icon="robot" href="/integrations/claude-hooks">
    The agent gate that feeds containment counts.
  </Card>
  <Card title="Environment variables" icon="gear" href="/configuration/environment">
    `FALLOW_IMPACT_STORE_MAX_AGE_DAYS` and the other Impact knobs.
  </Card>
</CardGroup>
