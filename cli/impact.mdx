...
When tracking is enabled and has non-zero results, fallow surfaces the headline numbers at most once a week: JSON output carries an `impact-report` entry in the [`next_steps[]` array](/integrations/mcp), and bare `fallow` prints a one-line `Impact:` summary with the run summary. The cadence stamp lives in the impact store, so the digest stays weekly across agents, sessions, and surfaces. Zero results never surface, and CI never sees the digest.
