...
| `--base <REF>` | Base branch for the hook (requires `--hooks`, default: auto-detected via git or `main`) |
...
- Runs `fallow check --changed-since <base> --fail-on-issues --quiet`
- Prints a success message that mentions `git commit --no-verify` for bypassing the hook

### Custom base branch

By default, the hook uses the auto-detected base branch (or `main` as fallback). Override with `--base`:

```bash
fallow init --hooks --base develop
```
