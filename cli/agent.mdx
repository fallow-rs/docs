...
`agent status` reports a different shape: `surfaces[]`, each with `harness`, `step`, `path`, and a `state` of `absent`, `installed`, or `stale`. `stale` means fallow wrote the surface but an older version did, or the surface is installed and would not do its job. A `hooks` gate written by an older fallow is `stale`, and so is an installed gate whose run-time prerequisites are missing, with the reason in `detail` and a matching entry in `next_actions[]`. Two gate conditions report that way: `gate-requires-jq`, because the gate script exits 0 after one stderr line when `jq` is absent and a PreToolUse hook never surfaces that, and `gate-path-version`, because the gate runs whatever `fallow` PATH resolves rather than the build that installed it. Branch on `state`, not on presence of the path.
