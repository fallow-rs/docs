...
`agent status` reports a different shape: `surfaces[]`, each with `harness`, `step`, `path`, and a `state` of `absent`, `installed`, or `stale`. `stale` means that an older fallow version wrote the surface, or that the surface is installed but would not do its job.
