...
To inspect or reclaim these caches on demand, see [Audit cache maintenance](#audit-cache-maintenance).
...
Minor and patch bumps and non-numeric ranges (workspace, file, git, tags) are never dependency candidates.
...
`deltas` also carries the dependency keys the decision surface reads: `dependency_added` (`<manifest>::<name>`) and `dependency_major_bumped` (`<manifest>::<name>@<from>-><to>`), so the envelope names what changed even when the cap collapses the decision.

`partition` carries `independent_slices`: the connected components of the inter-unit dependency graph, each a sorted list of module directories. Two or more slices mean the change splits along a graph-proven seam into pieces that can be reviewed and merged on their own. An orientation fact, never a demand to split.
...
| `rejected` (invalid action) | The `action` is outside the closed vocabulary. The judgment is refused before anchoring; fix the label. | `reason: "invalid-action"`. |
