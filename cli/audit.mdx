...
Returns `digest` (brief + decision surface), `direction`, `graph_snapshot_hash`, `agent_schema`, `injection_note`, and `emitted_signal_ids`. The digest is built from the graph only (PR prose is never folded in, so it is injection-resistant), and the snapshot hash is pinned into the digest.

**2. The agent returns judgment JSON.** Every `signal_id` must be one fallow emitted; echo the hash.
...
|:--------|:----------|:------|
| `accepted` | The `signal_id` is anchored and the snapshot still matches. | The judgment is returned with its framing fenced `deterministic: false`. |
