...
Every audit analyzes only the files that changed against its base, so a whole-project baseline matches less of the run for reasons that are not rot. Audit therefore loads the baselines you pass and judges none of them. Each loaded baseline gets its own `baseline_staleness` object in the envelope, at `dead_code.baseline_staleness`, `duplication.baseline_staleness` and `complexity.summary.baseline_staleness`. All of them report `change_scoped: true`, with `scope_reasons` naming what narrowed that sub-pass (`changed-since` for `dead_code`, `changed-files` for `duplication` and `complexity`). All of them report `gate_trips: false`, unless that section's baseline is one no command can read as its own. Do not build a gate on those values. To judge a baseline, run the command that owns it over the whole project: the section label is not the command name, so `duplication` points at `fallow dupes` and `complexity` at `fallow health`, while `dead_code` points at `fallow dead-code`.

An audit that loaded any baseline also carries `gate_outcomes["stale-baseline"]` as `{"status": "skipped", "enforced": false}`: one entry however many baselines were loaded, which is what puts the fact in the gate outcomes line the pull-request comment and merge-request note render. That entry stays `skipped` for a baseline of another `kind` too. See [CI](/integrations/ci) for what the shipped integrations report per baseline.

An audit checks the `kind` of every baseline it loads and reports every mismatch. Each mismatch gets its own note on stderr, naming the flag that carried the path: `--dead-code-baseline`, `--dupes-baseline` or `--health-baseline`. The matching section carries `baseline_staleness.unrecognised_format: true`. Point that flag at a baseline the owning command saved.

`--fail-on-stale-baseline` is accepted on `audit` and cannot fire. A run that sets it alongside a baseline says so once on stderr and names the commands that can run the gate instead.
