...
Every audit analyzes only the files that changed against its base, so a whole-project baseline matches less of the run for reasons that are not rot. Audit therefore loads the baselines you pass and judges none of them. Each loaded baseline gets its own `baseline_staleness` object in the envelope, at `dead_code.baseline_staleness`, `duplication.baseline_staleness` and `complexity.summary.baseline_staleness`. All of them report `change_scoped: true` and `gate_trips: false`, with `scope_reasons` naming what narrowed that sub-pass (`changed-since` for `dead_code`, `changed-files` for `duplication` and `complexity`). Do not build a gate on those values. To judge a baseline, run the command that owns it over the whole project: the section label is not the command name, so `duplication` points at `fallow dupes` and `complexity` at `fallow health`, while `dead_code` points at `fallow dead-code`.

An audit that loaded any baseline also carries `gate_outcomes["stale-baseline"]` as `{"status": "skipped", "enforced": false}`: one entry however many baselines were loaded, which is what puts the fact in the gate outcomes line the pull-request comment and merge-request note render. See [CI](/integrations/ci) for what the shipped integrations report per baseline.
