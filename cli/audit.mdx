...
| `--css-deep` | Include the opt-in structural CSS and CSS-in-JS analysis in audit. The audit still reports changed-file results, but this pass can read project stylesheets to classify token drift, duplicate declaration blocks, styling hot spots, and other styling findings with repository context. JSON adds `complexity.css_analytics` and first-class `complexity.styling_findings`; verdict severity follows the configured styling rules. |
...
| `complexity.styling_findings[]` | array | Present when CSS analysis is active, including `--css-deep`. Contains first-class styling issues with rule severity, actions, and introduced attribution when the base comparison is available. |
...
When `base` is not specified, the base is the `git merge-base` against the branch's upstream or the remote default (`origin/main`); set `FALLOW_AUDIT_BASE` in the server env to pin it. Set `gate` to `all` for strict mode. Set `css_deep` to `true` to pass `--css-deep` and include styling findings in the health sub-result. The response always includes `_meta` explanatory metadata (the MCP wrapper enables `--explain` by default). Returns the same JSON envelope as the CLI.
