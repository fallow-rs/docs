...
| Flag | Description |
|:-----|:------------|
| `--diff-file <PATH>` | Path to a pre-computed unified diff (e.g. `git diff --unified=0 main...HEAD > /tmp/pr.diff`) that narrows source-anchored findings to lines inside an added hunk. Pass `-` for stdin (`gh pr diff \| fallow audit --diff-file -`) or use the `--diff-stdin` alias. The bundled GitHub Action and GitLab CI template pre-compute the diff and pass it automatically; you only need this flag for non-PR pipelines or local invocations. The filtering rules and the `FALLOW_DIFF_FILE` fallback are documented under [global flags](/cli/global-flags#reference). |
...
| `--explain` | Add metric explanations in JSON output (`_meta` objects with docs links). In human output, also lists each clone group the new-only gate demoted, with the deciding diff source. See [New-only duplication demotion](#new-only-duplication-demotion). |
...
| `--walkthrough-file <PATH>` | Ingest an agent's judgment JSON and post-validate it against the live graph; see [Agent walkthrough loop](#agent-walkthrough-loop) for the accepted and rejected outcomes. Implies the brief; always exits 0. |
...
| `--coverage <PATH>` | Path to Istanbul-format coverage data (`coverage-final.json`) for accurate per-function CRAP scores in the health sub-analysis. Also configurable via `FALLOW_COVERAGE`, then `health.coverage` (the same chain as `fallow health`). Same format and semantics as [`fallow health --coverage`](/cli/health#coverage); without any of them, fallow auto-detects `coverage/coverage-final.json` or falls back to the static-estimate model. The resolved map feeds both the head pass and the base attribution pass. A configured file that does not exist exits 2, like `fallow health`. Relative paths resolve against `--root`. |
...
Every audit analyzes only the files that changed against its base. A whole-project baseline then matches less of the run, and the narrow scope is the reason rather than a stale baseline. An audit loads the baselines you pass and judges none of them. Each loaded baseline has its own `baseline_staleness` object in the envelope: `dead_code.baseline_staleness`, `duplication.baseline_staleness` and `complexity.summary.baseline_staleness`. All three report `change_scoped: true`. Their `scope_reasons` names what narrowed that sub-pass: `changed-since` for `dead_code`, and `changed-files` for `duplication` and `complexity`. All three report `gate_trips: false`. A section that loaded a baseline it cannot read as its own reports `true`. Do not build a gate on those values. To judge a baseline, run the command that owns it over the whole project. The section label is not the command name: `duplication` belongs to `fallow dupes`, `complexity` to `fallow health`, and `dead_code` to `fallow dead-code`.

An audit that loaded a baseline also carries `gate_outcomes["stale-baseline"]` as `{"status": "skipped", "enforced": false}`. There is one entry for any number of loaded baselines, and it puts the fact on the gate outcomes line. The pull-request comment and the merge-request note render that line. The entry stays `skipped` for a baseline of another `kind` too. See [CI](/integrations/ci) for what the shipped integrations report per baseline.

An audit checks the `kind` of every baseline it loads and reports every mismatch. Each mismatch warns on stderr and names the flag that carried the path: `--dead-code-baseline`, `--dupes-baseline` or `--health-baseline`. That section carries `baseline_staleness.unrecognised_format: true`. Point that flag at a baseline from the command that owns it.

`audit` accepts `--fail-on-stale-baseline`, and the gate always stands down. A run that sets the flag with a baseline warns once on stderr and names the commands that can run the gate.
...
When `base` is omitted it resolves as described under [How it works](#how-it-works); set `FALLOW_AUDIT_BASE` in the server env to pin it. Set `gate` to `all` for strict mode. Styling findings are included by default; set `css_deep` to `false` to skip project-wide styling reachability, or `true` to force it back on when config disables it. Returns the same JSON envelope as the CLI.
...
Each project root keeps one root-owned base-snapshot cache, rebuilt in place when the resolved base SHA changes, and the reuse lock is held for the whole audit so cleanup can never remove files under a running audit. Automatic age-based GC runs on every audit: the [`FALLOW_AUDIT_CACHE_MAX_AGE_DAYS`](/configuration/environment#fallow_audit_cache_max_age_days) env var (or the `audit.cacheMaxAgeDays` config field) tunes the 30-day default, and `0` disables the age-based sweep. Two subcommands maintain the caches on demand.

### `audit-cache remove`

To reclaim one project's caches unconditionally (warm or not), use `fallow audit-cache remove --root <PATH>`:
...
| `--max-age-days <N>` | Age threshold in days for this invocation. Precedence: this flag, then [`FALLOW_AUDIT_CACHE_MAX_AGE_DAYS`](/configuration/environment#fallow_audit_cache_max_age_days), then `audit.cacheMaxAgeDays`, then the 30-day default; the JSON output reports which source won. `0` disables age-based reclaim but still reclaims orphaned entries and entries whose recorded owner root is gone. |
...
The decision surface is the ranked, capped ([`--max-decisions`](#options)) set of `coupling-boundary`, `public-api-contract`, and `dependency` decisions the change embeds, each a framed judgment question anchored to a `signal_id`. [`fallow decision-surface`](/cli/decision-surface) emits it on its own and documents the categories, the fields each decision carries, and suppression.
...
`impact_closure` reports the blast radius as a magnitude, a sample, and a distribution rather than one long list. `affected_count` is the exact number of files the change reaches outside the diff, computed before any capping. `affected_not_shown` is a path-sorted sample of at most ten of them: a prefix of the sorted set, so it clusters in whichever directory sorts first and is meant for jumping into representative files, never for enumerating the reach or inferring its shape. `affected_by_dir` is the shape: `{ dir, count }` rows, heaviest directory first with the directory path breaking ties, at most twenty-five rows, with `affected_by_dir_omitted` counting the lighter directories that did not fit. Every row's count is exact. `coordination_gap` is separate and uncapped, and it is not a subset of the sample: it deliberately skips story and test consumers the affected set counts.
