...
Writes are all-or-nothing: a failure on one file (read-only directory, full disk) leaves every target file at its original content. `package.json` and `pnpm-workspace.yaml` are re-read at fix time instead, so a dependency or catalog entry that has already disappeared is a no-op.
