...
Unused enum members are also auto-fixed (the member is removed from the enum declaration). Unused files and class members are not auto-fixed. Use `fallow check` to identify those.
