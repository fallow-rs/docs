...
- With `--eager-only`, the walk follows only static imports that carry a runtime value. `import()`, lazy globs, worker loads, and `import type` do not qualify. The route explains why a module is in the eager group of [`fallow list --entry-weight`](/cli/list#startup-import-weight). When no such route exists, `reachable` is `false`.
