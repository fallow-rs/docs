...
`inspect` accepts the project and performance flags (`--root`, `--config`, `--workspace`, `--production`, `--no-cache`, `--threads`). See [global flags](/cli/global-flags) for the full list.
