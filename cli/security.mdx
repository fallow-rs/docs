...
| Flag | Description |
|:-----|:------------|
| `--runtime-coverage <PATH>` | Add production runtime state to `tainted-sink` candidates and use it as an extra ranking signal. Accepts a V8 coverage directory, a single V8 JSON file, or an Istanbul coverage map JSON file |
...
`finding_id` is a stable correlation id. It is identical across runs for the same rule, path, and line, and it matches the SARIF `partialFingerprints["fallowSecurity/v1"]` value of the finding. Two candidates of the same rule on the same line share one ID. Use it to track a candidate across runs (for example after a rebase) and to join JSON and SARIF output.
...
When two candidates in the candidate file have the same `finding_id`, `survivors` exits 2. This happens when two sinks of the same rule are on one line.
