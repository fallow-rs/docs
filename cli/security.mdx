...
Wiring a new untrusted source into an existing sink on a changed line counts as a new
candidate. Gated findings stay unverified: the human output says `REVIEW REQUIRED` (not
...
Exit codes: **8** = a new candidate was introduced; **0** = clean (or a docs-only / empty
...
A non-literal argument is a signal to verify: fallow does not prove the value is attacker-controlled or reaches the sink unsanitized.
...
Each finding includes `kind`, `path`, `line`, `col`, `evidence`, `trace`, `actions`, `severity`, and optional `reachability`. `severity` is a review-priority tier (`high`, `medium`, or `low`) derived from reachability, boundary, source-backed, and runtime-hot signals; it does not change gate or exit semantics. SARIF maps high and medium candidates to `warning`, and low candidates to `note`. `tainted-sink` findings additionally carry `category` (the catalogue id, for example `"dangerous-html"`) and `cwe` (the category's CWE number); `client-server-leak` findings omit both. `tainted-sink` findings can also include `reachability.untrusted_source_trace` when a module with a known untrusted source imports the sink module.
...
There is no `impact` field; `severity` is the only ranking fallow assigns.
