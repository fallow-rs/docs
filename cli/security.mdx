...
A run that wrote the document `--sarif-file` asked for carries a root [`request_outcomes`](/cli/global-flags#request-outcomes) object with the `sarif-file` entry as `applied`, naming the path, so a consumer can tell a run that produced the artifact from one that was never asked for it. A write that fails exits 2 with an error document instead, so no envelope reports that entry unapplied.
