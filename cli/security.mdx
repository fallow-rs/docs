...
A refactor that merely touches a file already containing a sink passes; a change that adds a
new sink, or wires a new untrusted source into an existing sink, on a changed line fails.
Gated findings stay unverified: the human output says `REVIEW REQUIRED` (not
`FAIL`), SARIF keeps every result at `level: note` with the verdict in
`run.properties.fallowGate`, and `--format json` carries an additive `gate` block
(`mode` / `verdict` / `new_count`). `newly-reachable` ignores unreachable candidates in both
trees; diff-only inputs (`--diff-file` / `--diff-stdin`) exit 2 for this mode.
When `newly-reachable` cannot map the analysis root into the temporary base
worktree, it warns on stderr and analyzes the whole worktree.

Exit codes: **8** = a new candidate was introduced in the changed lines; **0** = clean (or a docs-only / empty
diff); **2** = the gate could not compute the diff (an unfetched ref on a shallow clone, a bad
ref, not a git repo), loud rather than a silent pass. Exit 8 is dedicated and stable, so a
pipeline can soft-gate it without allow-listing real errors (GitLab
`allow_failure: exit_codes: [8]`).
...
Scope which catalogue categories run with `security.categories` in config:
...
A run that wrote the `--sarif-file` document carries a root [`request_outcomes`](/cli/global-flags#request-outcomes) object. The `sarif-file` entry there is `applied`, and it names the path the run wrote. You can then tell a run that wrote the document from a run that asked for none. A failed write makes the run exit 2 with an error document, so no envelope carries that entry as unapplied.
...
`candidate.network` is present only on `secret-to-network` candidates. It carries `destination`: the network call's URL when it is a static string literal (usually intended auth, the credential's own provider) or absent when the destination is dynamic (the higher-signal exfil case). Use it to triage exfil from intended auth without re-reading the code.
