...
<Note>
  Findings are candidates, not confirmed vulnerabilities. Fallow reports a structural trace, so an agent or a human can verify if the value can actually reach client-bundled code.
</Note>
...
`finding_id` is a stable correlation id. It is identical across runs for the same rule, path, and line, and it matches the SARIF `partialFingerprints` value of the finding. Use it to track a candidate across runs (for example after a rebase) and to join JSON and SARIF output.
