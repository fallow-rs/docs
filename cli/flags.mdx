...
| `--max-flag-age <DAYS>` | Exit 1 when a flag in scope is older than this many days. Opt-in. With `--flag-age off`, the command exits 2. |
...
A run with `--changed-since` or `--workspace` skips the gate and saves no baseline, because its counts do not compare with a whole-project baseline. The verdict is in `retirement.regression` in the JSON output.
...
- `vendor-only` rows need a whole-project run. A run with `--changed-since` or `--workspace` adds none.
...
| `retirement.max_flag_age` | With `--max-flag-age`: `max_days`, `exceeded` and the `flags[]` that are older, oldest first |
