...
Detect feature flag patterns in your codebase. Reports flag locations, detection confidence, and cross-references with dead code findings.
...
The `flags` fields (`sdkPatterns`, `envPrefixes`, `configObjectHeuristics`) are documented in the [configuration reference](/configuration/overview#flags). Custom `sdkPatterns` and `envPrefixes` are merged with the built-in sets, never replace them.
...
A run passed `--changed-since` also carries a root [`request_outcomes`](/cli/global-flags#request-outcomes) object with the `changed-since` entry, so a ref that would not resolve says so on the wire instead of silently widening the report to the whole project.
