...
A run with `--changed-since` also carries a root [`request_outcomes`](/cli/global-flags#request-outcomes) object with the `changed-since` entry. When fallow cannot resolve the ref, the entry says so and the report covers the whole project.
