...
| `--summary` | Print a one-line summary of the feature flag counts at the end of the run. In JSON format, adds a `summary` counts object. |
...
| `--fail-on-issues` | Exit with code 1 if fallow finds feature flags |
...
| `--fail-on-regression` | Fail if the feature flag count grows by more than the tolerance, compared with a regression baseline |
...
| `--save-regression-baseline <PATH>` | Save the current feature flag counts as a regression baseline |
