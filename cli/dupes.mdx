...
| `--top <N>` | Show only the N highest-ranked clone groups (see [Ranking and JSON fields](#ranking-and-json-fields)). `stats.clone_groups` and `stats.clone_instances` reflect the shown groups, while corpus totals and `stats.duplication_percentage` remain scoped to the full analysis. |
