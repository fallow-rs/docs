...
| Flag | Description |
|:-----|:------------|
| `--trace <FILE:LINE \| dup:<fp>>` | Deep-dive clones. `FILE:LINE` shows all clones at a source location; `dup:<8hex>` deep-dives a clone group by the stable fingerprint shown in the listing (and on `clone_groups[].fingerprint` in JSON). The trace shows each group's fingerprint, an extract-function suggestion with estimated savings, and a best-effort proposed name. |
