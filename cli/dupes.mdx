...
| `--ignore-imports` | Exclude import declarations from clone detection (default). Sorted import blocks are a formatting artifact, not copy-paste, so they are stripped from the token stream. Scope is ES `import` declarations; CommonJS `require()` calls and `export ... from` re-export blocks are still counted. |
| `--no-ignore-imports` | Count import declarations as clone candidates (opt out of the default import exclusion). Conflicts with `--ignore-imports`. |
