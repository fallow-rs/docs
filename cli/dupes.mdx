...
| `--top <N>` | Show only the N highest-ranked clone groups. Ranking multiplies clone size by occurrence count, then applies a capped spread boost. `stats.clone_groups` and `stats.clone_instances` reflect the shown groups, while corpus totals and `stats.duplication_percentage` remain scoped to the full analysis. |
...
If your duplication percentage drops after upgrading, fallow is now excluding generated framework output and test/mock files from duplicate detection by default. Use `--explain-skipped` in human or markdown output to see the per-pattern skipped-file counts.
