...
| `excluded-by-default-ignore` | A built-in discovery ignore pattern (`**/dist/**`, `**/build/**`, `**/coverage/**` or one of the minified-bundle globs) removed at least one candidate source file from the walk. One entry per pattern, never per file, with the `pattern`, an exact `file_count`, a `directory_count` of the distinct locations the pattern matched at, and `path`, the project-relative matched directory that lost the most files, anchored at the deepest matching segment so `fallow --root <path>` recovers them. `**/node_modules/**` is never reported. Emitted by source discovery, so it appears in `dead-code`, `check`, `audit` and `health` JSON; `--explain-skipped` prints the same counts on stderr, and a run that discovered no source files at all prints one warning without the flag. |
| `bun-lockb-override-resolution-skipped` | The root `package.json` declares overrides, bun's legacy binary `bun.lockb` sits next to it, and no parseable text lockfile (`bun.lock`, `pnpm-lock.yaml`, `package-lock.json`, `npm-shrinkwrap.json`) was found to use instead, so the `unused-dependency-overrides` check is skipped and reports nothing. A `yarn.lock` or an unparseable `bun.lock` beside the `bun.lockb` does not prevent the skip. Run `bun install --save-text-lockfile` (bun 1.2 or newer) to write a text `bun.lock`, or delete the stale `bun.lockb` if the repository no longer uses bun. Emitted by the override analysis rather than workspace discovery, so it appears in `dead-code`, `check`, and `health` JSON and as one stderr warning. |

A malformed ROOT `package.json` is treated as a hard config-load error and exits 2 instead of warning; everything else is warn-and-continue so analysis still runs on the remaining workspaces. The same workspace-discovery diagnostics are emitted on `fallow dead-code`, `fallow dupes`, and `fallow health` JSON envelopes (omitted when empty), so CI consumers can gate on discovery problems without a separate command. Those analysis envelopes can also include source-discovery skip diagnostics: `skipped-large-file` and `skipped-minified-file`, each with `size_bytes`, and `excluded-by-default-ignore` for files a built-in ignore pattern removed.
...
tooling: `auto_discover` (verbatim user strings), `source_zone_index` (position
in the pre-expansion `zones[]`), `status` (`ok` / `empty` / `invalid_path`),
`authored_rule` (the pre-expansion rule keyed on the parent), `fallback_zone` (a
back-reference when the parent kept its own `patterns` and stayed in `zones[]`),
`merged_from` (parent zone indices when the same parent name was declared
twice), `original_zone_root` (echo of the parent's `root` subtree scope), and
`child_source_indices` (parallel to `children`, attributing each child to a
specific `auto_discover` entry). The full shape is documented in
