...
<Tip>
  Use `fallow list --plugins` to verify which of the 118 built-in plugins are active for your project.
</Tip>
...
A malformed ROOT `package.json` is treated as a hard config-load error and exits 2 instead of warning; everything else is warn-and-continue so analysis still runs on the remaining workspaces. The same `workspace_diagnostics[]` array is emitted on `fallow dead-code`, `fallow dupes`, and `fallow health` JSON envelopes (omitted when empty), so CI consumers can gate on discovery problems without a separate command.
