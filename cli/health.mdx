...
<Note>
  Without file-scores enabled, the `vital_signs` object is omitted from the JSON output. The `duplication_pct` field is `null` when duplication analysis (`fallow dead-code` or `fallow dupes`) has not been run.
</Note>
