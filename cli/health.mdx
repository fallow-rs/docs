...
```json title="$ fallow health --format sarif (excerpt)"
{
  ...
  "version": "2.50.0",
  "runs": [{
    "tool": { "driver": { "name": "fallow", "version": "2.50.0" } },
    "results": [
      {
...
For findings triggered by CRAP, the primary action is selected based on a bucketed `coverage_tier` field on each finding:
...
- `high` (Istanbul `> 70`, or estimated 85% band) AND CRAP-only: emits `refactor-function` instead of a coverage action, because at high coverage additional tests cannot meaningfully reduce CRAP. Reducing cyclomatic complexity is the only remaining lever.

When CRAP-only and the function's cyclomatic count is within 5 of `maxCyclomatic`, a secondary `refactor-function` action is also emitted alongside the coverage action.
