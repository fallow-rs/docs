...
Analyze function complexity and file-level health across your codebase. By default, fallow reports all four sections: complexity findings, file health scores, hotspot analysis, and refactoring targets. Use `--complexity`, `--file-scores`, `--hotspots`, or `--targets` to select individual sections.

<Tip>
  All four sections are enabled by default. Use `--complexity`, `--file-scores`, `--hotspots`, or `--targets` to select individual sections. Add `--top N` to limit results.
</Tip>
...
| `--score` | Show only the project health score (0-100) with letter grade (A/B/C/D/F). Forces full pipeline (file-scores + hotspots) for maximum accuracy. |
...
<Tip>
  Store snapshots in CI artifacts or commit them to your repo to build a history of codebase health over time. Combine `--save-snapshot` with `--score` to include the health score and grade in the snapshot for trend tracking.
</Tip>
