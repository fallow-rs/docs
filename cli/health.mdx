...
Each command writes its own baseline format. `fallow health --baseline` loads a file saved by another command, or an empty file, as zero entries, so nothing can go unmatched and the gate would stay green on the wrong file. Such a run warns on stderr, with or without `--quiet`, and reports `summary.baseline_staleness.unrecognised_format: true`. The member is present only when true, so read it as absent-means-false, and read it rather than `baseline_entries == 0`: a baseline saved from a project with nothing above threshold is legitimately empty. The check compares the keys in the file against the keys `--save-baseline` writes. Exit codes do not move.
