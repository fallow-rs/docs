...
| `--ownership` | Attach ownership signals to each hotspot entry: bus factor (Avelino truck factor), contributor count, top contributor with stale-days, recent contributors (top-3), declared CODEOWNERS owner, ownership drift, and unowned-hotspot detection. Implies `--hotspots`. Requires a git repository. |
...
|:------------|:-------------|:--------------------|
| `low-bus-factor` | `bus_factor == 1` | Add a second reviewer to the file. |
| `unowned-hotspot` | `unowned == true` (CODEOWNERS exists, no rule matches) | Add a CODEOWNERS entry. The action's `suggested_pattern` field offers a sensible default (e.g. `/src/api/`). |
...
```json title="$ fallow health --format sarif (excerpt)"
{
  ...
  "version": "2.1.0",
  "runs": [{
    "tool": { "driver": { "name": "fallow", "version": "2.37.0" } },
    "results": [
      {
