...
| `--css` | Add a structural CSS analytics section (`css_analytics` in JSON, `CSS health` in human, `## CSS Health` in markdown). Surfaces codebase-scale CSS slop that per-rule linters do not aggregate: specificity hotspots (id selectors, deep compound selectors), `!important` density, over-complex selectors, deep nesting, empty rules, and design-token sprawl (the count of distinct color, `font-size`, `z-index`, `box-shadow`, `border-radius`, and `line-height` values across the whole codebase). Cleanup candidates: custom properties (`--x`) and `@keyframes` defined but never referenced, dead Vue `<style scoped>` classes, unused `@property` / `@layer` at-rules, and (on Tailwind projects) arbitrary-value bypasses like `w-[13px]`. It also flags the inverse, references that resolve to nothing: an `animation` whose `@keyframes` is defined nowhere, plus duplicate declaration blocks (4+ identical declarations repeated across rules) as copy-paste consolidation candidates. Each finding carries a read-only verification step; everything is framed as a candidate (a token can still be used from JavaScript or inline HTML), never a gated finding. Opt-in because it reads and parses every project stylesheet; Vue/Svelte SFC `<style>` blocks are analyzed alongside `.css` files. Standard CSS only (SCSS is skipped). |
...
```json title="$ fallow health --format sarif (excerpt)"
{
  ...
  "version": "2.96.0",
  "runs": [{
    "tool": { "driver": { "name": "fallow", "version": "2.96.0" } },
    "results": [
      {
