---
title: "fallow health"
description: "CLI reference for fallow health. Analyze function complexity, per-file maintainability, git-churn hotspots, refactoring targets, static coverage gaps, and runtime coverage."
...
| `--coverage-gaps` | Show runtime files and exports that no test dependency path reaches. Mock-aware; see [With coverage gaps](#with-coverage-gaps). Opt-in (default off). Configure severity via the `coverage-gaps` rule. |
| `--coverage <PATH>` | Path to Istanbul-format coverage data (`coverage-final.json`) for accurate per-function CRAP scores. Produced by Jest, Vitest, c8, nyc. When provided, uses the canonical formula `CC^2 * (1-cov/100)^3 + CC` instead of the static binary model. Relative paths resolve against `--root`. Falls back to `FALLOW_COVERAGE`, then `health.coverage`, then auto-detection; [`fallow audit`](/cli/audit) uses the same chain. Statement coverage is computed per function over the statements in its own body: a nested function's statements belong to the nested function, not to the one that contains it. |
...
Add repeatable `--type-aware-project <PATH>` flags when automatic project
...
| Flag | Description |
|:-----|:------------|
| `--save-baseline` | Save current complexity findings as a baseline for future comparison. Fallow refuses the save with exit 2 when the destination holds a baseline of another `kind`. A destination with no `kind` gains one on the next save. |
...
| `--fail-on-stale-baseline` | Exit 1 when the loaded baseline has any entry that matched no current finding; see [global flags](/cli/global-flags). Stands down on `--report-only`, which is an explicit request never to fail. |

When a quarter or more of the saved entries match no current finding, the run warns on stderr and gives the re-save command. A run narrowed by `--changed-since`, `--diff-file`, `--workspace`, `--changed-workspaces` or `--production` stays silent, because it cannot judge a whole-project baseline.

On an analysis run, `summary.baseline_staleness` reports how much of the baseline still matches (`fallow report --from` re-renders a saved envelope and carries it only if that envelope already had it): the counts, `moved_entries`, `current_findings`, `change_scoped`, the advisory verdict as `warning`, and `gate_trips`, which is the flag's own rule. Read `change_scoped` before dividing the counts: a narrowed run can report `matched_entries: 0` on a healthy baseline.

`scope_reasons` names what narrowed the run, so you do not have to guess which of your inputs is responsible. It is present and not empty exactly when `change_scoped` is true. `health` compares after the flags resolve, so it reports `diff`, `changed-files` for a resolved changed-file set, `workspace` for both `--workspace` and `--changed-workspaces`, and `production`. Each command reports what it can see, so the names differ per command. Read the set as open.

A run can load a baseline with entries and use only a diff or `--changed-since`. Such a run also gains a `recheck-baseline` entry in `next_steps[]` with the unscoped command. That command re-reads the baseline and reports. It never re-saves the baseline. A run in production mode, or a run scoped to workspaces, gains no entry. Those channels resolve from config and from the environment as well as from a flag. The command in the entry would then be just as narrow. A diff-scoped run gains no entry while you export `FALLOW_DIFF_FILE`, for the same reason.

Each command writes its own baseline format, and a saved baseline names that format. Every file `--save-baseline` writes carries a top-level `kind` of `dead-code`, `dupes` or `health`. A baseline that an earlier release saved carries no `kind` and still loads. For such a file the keys it carries decide the format.

`fallow health --baseline` loads a baseline of another kind as zero entries, so no entry stays unmatched. Such a run warns on stderr, with or without `--quiet`, and names both kinds and the path. It reports `summary.baseline_staleness.unrecognised_format: true` and suppresses nothing. The member is present only when it is true, and an absent member means false. Read the member rather than `baseline_entries == 0`, because a baseline from a project with nothing above threshold is correctly empty. The baseline gate trips on such a file, so `--fail-on-stale-baseline` exits 1 there. A run that armed no gate keeps its exit code.
...
File scores are ordered by risk-aware triage concern: the larger of low-MI concern and CRAP risk. This keeps files with very high untested complexity near the top even when their Maintainability Index is not the lowest. The formula and its components are documented under [File health scores](/explanations/health#file-health-scores).
...
The hotspot score identifies files that are both complex and frequently changed; the formula, the recency weighting, and the trend rules are documented under [Hotspot metrics](/explanations/health#hotspot-metrics).
...
| `shallow_clone` | Whether a shallow clone was detected. Shallow history produces incomplete results; run `git fetch --unshallow` for a full clone. |
...
A health input that failed to load contributes zeros, and a zero reads like a clean measurement. To tell a measured zero from a missing measurement, read the number together with `workspace_diagnostics[]`. A diagnostic with `degrades_analysis: true` beside a zero means fallow measured nothing.

These inputs report that way:
...
The inputs that stop churn share one kind, `hotspots-skipped`, and its `cause` names the input: `not-a-repository`, `invalid-since` or `churn-file-unreadable`. `coverage-auto-detected` is the exception. It names the coverage file that fed the CRAP scores, so it reports provenance for an input that did load, and it leaves `degrades_analysis` unset. [Debugging](/analysis/debugging#a-health-section-reports-nothing) lists every kind, and which numbers each kind leaves at zero.

Select on `degrades_analysis`, not on a list of kinds, so a kind added later reaches your consumer unchanged:
...
Complexity findings evaluated with local ceilings include `effective_thresholds` and `threshold_source: "override"`. When configured threshold overrides affect a run, JSON also includes a top-level `threshold_overrides[]` array; see [Threshold overrides](#threshold-overrides) below.
...
| `override_index` | Position of the entry in the configured `thresholdOverrides` array. Shared by every row the entry produces. |
| `dimension` | Threshold dimension this row describes: `complexity` or `crap`. |
| `outstanding` | Dimensions the matched unit still breaches despite the override, whether or not the entry configures their ceilings. Omitted when empty. |
| `path`, `function` | Matched file, and matched function for function-scoped entries. |
| `line`, `col` | 1-based line and 0-based column of the matched unit. Absent on `no_match` rows. Several units in one file can share a name; the position keeps their rows distinct. |
| `configured_thresholds` | Ceilings the entry itself configures. |
| `effective_thresholds` | Ceilings in effect for the matched unit after applying every matching entry on top of the defaults. |
| `metrics` | Current `cyclomatic` and `cognitive` of the matched code, plus `crap` when coverage data exists. Absent on `no_match` rows. |
| `reason` | The entry's configured `reason`, when set. |
...
When file-scores are enabled (via `--file-scores` or implicitly via `--save-snapshot`), the JSON output includes a top-level `vital_signs` object with the metrics listed under [Vital signs](#vital-signs), plus the nested `counts` object. The [snapshot format](#snapshot-format) shows a complete example.
...
The score is reproducible from the penalties: `100 - sum(penalties) == score`. Penalty fields are `null` (absent from JSON) when the corresponding pipeline didn't run. The penalty formulas, `formula_version`, and the letter-grade bands are documented under [Health score](/explanations/health#health-score).
...
Use [`fallow coverage setup`](/cli/coverage) for first-run capture instructions; start a trial with [`fallow license`](/cli/license) when you need continuous or multi-capture runtime monitoring. For the conceptual model and trade-offs, see [Runtime coverage](/analysis/runtime-coverage).
