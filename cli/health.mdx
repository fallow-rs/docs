...
```json title="$ fallow health --production-coverage ./coverage --format json"
{
  ...
  "production_coverage": {
    ...
    "summary": {
      "functions_total": 128,
      "functions_called": 93,
      "functions_never_called": 21,
      "functions_coverage_unavailable": 14,
      "percent_dead_in_production": 16.4
    },
    "findings": [
      {
...
| `summary` | Aggregate counts of tracked, called, cold, and unresolved functions. |
| `findings` | Cold or unresolved functions. The `state` field is one of `called`, `never-called`, `coverage-unavailable`, or `unknown`. |
| `hot_paths` | Highest-invocation runtime functions, filtered by `--min-invocations-hot`. |
...
```json title="$ fallow health --format sarif (excerpt)"
{
  ...
  "version": "2.1.0",
  "runs": [{
    "tool": { "driver": { "name": "fallow", "version": "2.38.0" } },
    "results": [
      {
