...
| Flag | Description |
|:-----|:------------|
| `--save-baseline` | Save current complexity findings as a baseline for future comparison. A save is refused with exit 2 when the destination holds a baseline of another `kind`. A destination that carries no `kind` gains one on the next save. |
...
When a quarter or more of the saved entries match no current finding, the run prints a partial-staleness warning on stderr with the re-save command. A run narrowed by `--changed-since`, `--diff-file`, `--workspace`, `--changed-workspaces` or `--production` stays silent, because it cannot judge a whole-project baseline.
...
`scope_reasons` names what narrowed the run, so you do not have to guess which of your own inputs is responsible. It is present and non-empty exactly when `change_scoped` is true. `health` compares after the flags resolve, so it reports `diff`, `changed-files` for a resolved changed-file set, `workspace` for both `--workspace` and `--changed-workspaces`, and `production`. Each command reports what it can see, so the names differ per command, and treat the set as open.

A run that loaded a non-empty baseline and was narrowed only by a diff or `--changed-since` also gains a `recheck-baseline` entry in `next_steps[]`, carrying the unscoped command. That command re-reads the baseline and reports, and never re-saves it. A run in production mode or scoped to workspaces gets no entry, because those resolve from config and the environment as well as from a flag, so the suggested command would come back just as narrow. Neither does a diff-scoped run while `FALLOW_DIFF_FILE` is exported, for the same reason.

Each command writes its own baseline format, and a saved baseline names its own. Every file `--save-baseline` writes carries a top-level `kind` of `dead-code`, `dupes` or `health`. A baseline that an earlier release saved carries no `kind` and still loads. For such a file the keys it carries decide the format.

`fallow health --baseline` loads a baseline of another kind as zero entries, so nothing can go unmatched. Such a run warns on stderr, with or without `--quiet`, and names both kinds and the path. It reports `summary.baseline_staleness.unrecognised_format: true` and suppresses nothing. The member is present only when true, so read it as absent-means-false. Read it rather than `baseline_entries == 0`: a baseline saved from a project with nothing above threshold is legitimately empty. The baseline gate trips on such a file, so `--fail-on-stale-baseline` exits 1 there. A run that armed no gate keeps its exit code.
...
A health input that failed to load contributes zeros, which reads the same as a clean measurement. To tell "measured zero" from "measured nothing", pair the number with `workspace_diagnostics[]`: a zero next to a diagnostic that carries `degrades_analysis: true` was not measured. A failed scoring pass, a project with no git repository, a `--since` window fallow could not read, a `--churn-file` that became unreadable after validation, a shallow clone, an unpinned clock, an unparsable CODEOWNERS file and an unreadable trend snapshot all report that way. The three that stop churn share one kind, `hotspots-skipped`, and its `cause` names which input it was: `not-a-repository`, `invalid-since` or `churn-file-unreadable`. `coverage-auto-detected` is the exception: it names the coverage file that fed the CRAP scores, so it reports provenance for an input that did load and leaves `degrades_analysis` unset. Every kind, and which numbers each one hollows out, is in [debugging](/analysis/debugging#a-health-section-reports-nothing).
...
Every entry is written whether or not `--quiet` is set, so a CI job that drops stderr still sees it.
