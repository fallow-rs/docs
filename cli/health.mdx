...
Each target includes `effort` (`low`/`medium`/`high`) and `factors` with raw `value`/`threshold` for programmatic use. The `evidence` field provides actionable detail for supported categories (`remove_dead_code`, `extract_complex_functions`, `break_circular_dependency`) — omitted for other categories.

<Note>
  Without `--targets`, the `targets` array is omitted entirely from the JSON output.
</Note>
