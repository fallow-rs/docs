---
title: "fallow health"
description: "CLI reference for fallow health. Analyze function complexity, per-file maintainability, git-churn hotspots, refactoring targets, static coverage gaps, and runtime runtime coverage."
...
Angular `.html` templates are scanned alongside JavaScript and TypeScript functions: when a template uses control-flow blocks (`@if`, `@for`, `@switch`, `@case`, `@defer (when ...)`, `@let`), legacy structural directives (`*ngIf`, `*ngFor`), or expression-bound attributes (`[x]`, `(x)`), fallow emits a synthetic `<template>` finding with cyclomatic and cognitive scores. Suppress with an HTML comment at the top of the template: `<!-- fallow-ignore-file complexity -->`.
...
`--runtime-coverage` accepts a V8 directory, a single V8 JSON file, or a single Istanbul coverage map JSON file. For the intended first-run flow, start with [`fallow license`](/cli/license), then run [`fallow coverage setup`](/cli/coverage). For the conceptual model and trade-offs, see [Runtime coverage](/analysis/runtime-coverage).
