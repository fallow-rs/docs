...
Analyze function complexity and file-level health across your codebase. By default, fallow reports all three sections: complexity findings, file health scores, and hotspot analysis. Use `--complexity`, `--file-scores`, or `--hotspots` to select individual sections.

<Tip>
  All three sections (complexity, file scores, hotspots) are enabled by default. Use `--complexity`, `--file-scores`, or `--hotspots` to select individual sections. Add `--top N` to limit results.
</Tip>
