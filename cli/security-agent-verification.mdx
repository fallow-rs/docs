...
`attack_surface.defensive_boundary.controls` lists control patterns observed in files on the import trace. Their presence does not prove that a control executes before the reported sink or protects the same input. Check execution order and value flow in the source windows before using a control to dismiss a candidate.
...
Finding IDs include the rule, project-relative path, line and column so separate sinks on one line stay distinct. JSON and the visualization share this ID with security SARIF's `partialFingerprints["fallowSecurity/v2"]`. Selecting fewer files does not change the IDs of the findings that remain.

When upgrading from line-only IDs, regenerate candidate files and verdicts together. Refresh evaluation labels keyed by `finding_id` too. Every security finding gets a new ID, including findings that did not collide before.

Keep historical candidate/verdict pairs together for replay. Do not match their verdicts to fresh candidates by path and line alone. Stored review history does not transfer automatically, and SARIF consumers may close and reopen security alerts once. The legacy `fallowSecurity/v1` fingerprint is no longer emitted because it could merge distinct sinks again.
