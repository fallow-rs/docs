...
Each command writes its own baseline format. A file saved by another command, or an empty file, does not load here: `fallow dead-code --baseline` fails to parse it and exits 2, because several dead-code baseline fields have no default. `fallow dupes` and `fallow health` load such a file as zero entries and report `baseline_staleness.unrecognised_format: true` instead.
