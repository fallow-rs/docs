...
| `--re-export-cycles` | Only report re-export cycles (barrel files re-exporting from each other in a loop, or self-loops). See [explanations/dead-code#re-export-cycles](/explanations/dead-code#re-export-cycles) |
| `--boundary-violations` | Only report boundary violations |
| `--policy-violations` | Only report rule-pack policy violations (banned calls and banned imports declared via the `rulePacks` config key). See [explanations/dead-code#policy-violations](/explanations/dead-code#policy-violations) |
...
| `--include-entry-exports` | Also report unused exports in entry files (package.json `main`/`exports`, framework pages, etc.), which are otherwise assumed to be consumed externally. See [global flags](/cli/global-flags#reference). |
...
See [Type-aware TypeScript analysis](/analysis/type-aware) for the safety
...
| `--save-baseline <PATH>` | Save current results as a baseline file. Fallow refuses the save with exit 2 when the destination holds a baseline of another `kind`. A destination with no `kind` gains one on the next save. |
| `--fail-on-stale-baseline` | Exit 1 when the loaded baseline has any entry that matched nothing this run; see [global flags](/cli/global-flags) |
...
When a quarter or more of the saved baseline entries match no current issue, the run prints a partial-staleness warning on stderr with the re-save command, the same way `health --baseline` does. A run narrowed by `--file`, `--changed-since`, `--diff-file`, `--workspace`, `--changed-workspaces`, a positional path, `--production` or an `--unused-*` filter stays silent, because it cannot judge a whole-project baseline. `--fail-on-stale-baseline` turns any stale entry into exit 1. The analysis run's JSON output carries `baseline_staleness` whenever a baseline was loaded (`fallow report --from` re-renders a saved envelope and carries it only if that envelope already had it): the counts, `current_findings`, `change_scoped`, the advisory verdict as `warning`, and `gate_trips`, which is this flag's own rule so a CI integration reads one boolean instead of restating it. Read `change_scoped` before dividing the counts: a narrowed run can report `matched_entries: 0` on a healthy baseline.

`scope_reasons` names what narrowed the run, so you do not have to guess which of your inputs is responsible. It is present and not empty exactly when `change_scoped` is true. `dead-code` reads the flags and reports `diff`, `changed-since`, `workspace`, `changed-workspaces`, `scope`, `file`, `issue-type-filter` and `production`. `issue-type-filter` is an active `--unused-*` filter, which drops whole baseline categories before the comparison. Each command reports what it can see, so the names differ per command. Read the set as open. Use `scope_reasons` to see whether a repeat run can drop the scope: `production`, `workspace` and `changed-workspaces` state what you think the project is, so a repeat keeps them.

A run can load a baseline with entries and use only the channels a repeat can drop: `diff`, `changed-since`, `scope`, `file` and `issue-type-filter`. Such a run also gains a `recheck-baseline` entry in `next_steps[]` with the unscoped command. That command re-reads the baseline and reports. It never re-saves the baseline. A run in production mode, or a run scoped to workspaces, gains no entry. Those channels resolve from config and from the environment as well as from a flag. The command in the entry would then be just as narrow. A diff-scoped run gains no entry while you export `FALLOW_DIFF_FILE`, for the same reason. A bare `fallow` run follows the same rule and carries the same entry.
...
`fallow dead-code --baseline` loads a baseline of another kind as zero entries, so no entry stays unmatched. Such a run warns on stderr, with or without `--quiet`, and names both kinds and the path. It reports `baseline_staleness.unrecognised_format: true` and suppresses nothing. The member is present only when it is true, and an absent member means false. Read the member rather than `baseline_entries == 0`, because a baseline from a project with nothing to record is correctly empty. The baseline gate trips on such a file, so `--fail-on-stale-baseline` exits 1 there. A run that armed no gate keeps its exit code. Invalid JSON still exits 2, and a dead-code baseline that carries only part of its own keys exits 2 as well.
...
`files` is sorted lexicographically; the `multi-node` shape carries two or more entries, the `self-loop` shape exactly one. See [explanations/dead-code#re-export-cycles](/explanations/dead-code#re-export-cycles) for the structural rationale.
...
| `value_schema` | _(optional)_ URL pointing at the JSON Schema fragment that describes `value`. Agents that want to validate the payload before writing it into a user's config can fetch the linked schema and apply it |
