...
A run passed `--changed-since` also carries a root [`request_outcomes`](/cli/global-flags#request-outcomes) object with the `changed-since` entry, so a ref that would not resolve says so on the wire instead of silently widening the inventory to the whole project.
