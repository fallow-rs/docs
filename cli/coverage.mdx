...
| `--top <N>` | Show only the top N runtime findings and hot paths. Truncation happens before rendering, so it propagates to JSON, human, and cloud-merge output equally. |
...
| `--repo <NAME>` | `package.json` `repository.url`, then `git remote get-url origin` | Repo name used in `/v1/coverage/:repo/source-maps`. Auto-detection trims common git URLs to the repo name. |
