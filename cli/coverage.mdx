...
description: "CLI reference for fallow coverage. Run the resumable setup flow, analyze local or cloud runtime coverage, upload source maps, and push static inventory so the dashboard can surface untracked code."
...
| `--json` | Emit deterministic agent-readable setup instructions as JSON. Implies `--non-interactive` and does not prompt, write files, install packages, activate a license, or make network calls. See [Agent-readable JSON](#agent-readable-json). |
...
| `--debug-unmatched` | List every cloud runtime function with no local counterpart on stderr, highest traffic first. Use it to see what the join dropped without attaching a debugger. |
...
| `--with-callers` | Also upload importer edges (which modules import each declaration) alongside the inventory, so the dashboard can surface caller reach for untracked code. The inventory rows also carry per-function complexity and per-file churn. |
