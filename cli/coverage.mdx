...
`fallow coverage setup --yes --json` is the agent-driven entry point. The payload is deterministic, side-effect-free, and stable across reruns. On a workspace monorepo (Elysia API at root + two Vite browser apps in `dashboard/` and `site/`):
...
`framework_detected` uses canonical ids (`nextjs`, `nestjs`, `nuxt`, `sveltekit`, `astro`, `remix`, `vite`, `plain_node`, `unknown`). The top-level `dockerfile_snippet` mirrors the root member, so on a browser-rooted monorepo with a Node child, agents should read each member's `dockerfile_snippet` rather than the top-level one. Single-app projects emit a `members` array of length 1 (path `"."`), so consumers can treat `members[]` uniformly.
