...
<Tip>
  122 built-in plugins cover Next.js, Vite, Ember, Wuchale, Jest, Tailwind, PandaCSS, Contentlayer, tap, tsd, and more. No configuration needed for the first run.
</Tip>
...
This assumes `fallow-mcp` is on your `PATH`. If fallow is a project devDependency, the binary lives in `node_modules/.bin/` instead, so launch it through your package manager: set `"command": "npx"` with `"args": ["fallow-mcp"]` (or `pnpm exec` / `yarn` / `bunx`). See the [MCP integration guide](/integrations/mcp).
