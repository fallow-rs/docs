...
Path to Istanbul coverage data (`coverage-final.json`) used for accurate per-function CRAP scores in `fallow health` and `fallow audit`. The `--coverage` CLI flag wins when both are set.
