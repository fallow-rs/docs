...
| `FALLOW_CLOCK_EPOCH` | Pin the single reference instant a run measures churn recency, hotspot ranking, and ownership staleness against, so two runs over one commit produce the same churn-derived numbers. Read the value a run resolved from `hotspot_summary.clock.epoch_secs` and pass it back | Unix epoch seconds |
