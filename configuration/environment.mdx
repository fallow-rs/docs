...
| `FALLOW_TELEMETRY` | Opt-in [telemetry](/cli/telemetry) mode (off by default) | `off`/`on`/`inspect` (plus `0`/`1`/`true`/`false`/`disabled`/`enabled`/`debug`/`log`) |
...
| `FALLOW_AGENT_SOURCE` | Declare the calling agent for [telemetry](/cli/telemetry#agent-source) classification | `codex`, `claude_code`, `cursor`, `copilot`, `opencode`, `aider`, `roo`, `windsurf`, `gemini`, `cline`, `continue`, `zed`, `goose`, `other_known`, `unknown`, `none` |
