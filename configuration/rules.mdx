...
Most issue types default to `error` when not configured. The exceptions are `private-type-leaks` and `coverage-gaps` (default `off`), plus `stale-suppressions` and `policy-violation` (default `warn`).
