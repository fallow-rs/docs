...
A pack file declares `version: 1`, a unique `name`, and a non-empty `rules` array. `banned-call` rules match callee paths segment-aware and import-resolved (`child_process.*` covers named, namespace, and default imports from `child_process` and `node:child_process`); `banned-import` rules match raw import specifiers segment-aware (`moment` covers `moment/locale/nl`, never `moment-timezone`). Rules scope with optional `files` / `exclude` globs, skip type-only imports with `"ignoreTypeOnly": true`, and carry an optional per-rule `severity` overriding the `rules."policy-violation"` master (default `warn`).
