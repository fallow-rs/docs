...
| `ignoreImports` | `boolean` | `true` | Exclude ES `import` declarations from clone detection. Sorted import blocks are a formatting artifact, not copy-paste, so they are excluded by default; set to `false` to count them again. Only affects ES `import` statements; CommonJS `require()` calls and `export ... from` re-export blocks are still counted. |
