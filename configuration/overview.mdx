...
Incremental cache tuning. Today the only knob is `maxSizeMb`, which caps the on-disk extraction cache and triggers LRU eviction during save.
...
Default cap: 256 MB. The cap is the size of the serialized `.fallow/cache.bin`; saves that cross 80% of the cap evict the oldest entries down to 60%. The env var `FALLOW_CACHE_MAX_SIZE` overrides this field when both are set. Both values are interpreted as whole megabytes.
