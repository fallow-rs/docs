...
List declarative rule packs: standalone JSON or JSONC files of `banned-call` and `banned-import` rules, loaded as pure data (no project code ever executes). Paths are project-root-relative. Matches report as `policy-violation` findings identified by `<pack>/<rule-id>`.
