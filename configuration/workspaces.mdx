...
Fallow discovers `tsconfig.json` files in each workspace package automatically. Path aliases defined in each package's `tsconfig.json` are used when resolving imports within that package.
